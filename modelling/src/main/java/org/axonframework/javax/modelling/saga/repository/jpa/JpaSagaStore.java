 ...
 * @since 3.0
 * @deprecated in favor of using {@link org.axonframework.modelling.saga.repository.jpa.JpaSagaStore} which moved to
 * jakarta.
 */
@Deprecated
public class JpaSagaStore implements SagaStore<Object> {
    ...
    protected Set<AssociationValue> loadAssociationValues(EntityManager entityManager, Class<?> sagaType,
        ...
        List<org.axonframework.javax.modelling.saga.repository.jpa.AssociationValueEntry> associationValueEntries =
                entityManager.createNamedQuery(FIND_ASSOCIATIONS_NAMED_QUERY,
                                               org.axonframework.javax.modelling.saga.repository.jpa.AssociationValueEntry.class)
                             .setParameter("sagaType", getSagaTypeName(sagaType))
                             .setParameter("sagaId", sagaIdentifier)
                             .getResultList();

        return associationValueEntries.stream()
    ...
    protected void storeAssociationValue(EntityManager entityManager, Class<?> sagaType, String sagaIdentifier,
                                         AssociationValue associationValue) {
        entityManager.persist(new AssociationValueEntry(
                getSagaTypeName(sagaType),
                sagaIdentifier,
                associationValue));
    }
