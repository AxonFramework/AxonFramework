...
public class DocumentPerCommitStorageStrategy implements StorageStrategy {
    ...
    public List<Document> createDocuments(String type, Serializer eventSerializer, List<DomainEventMessage> messages) {
        return Collections.singletonList(new CommitEntry(type, eventSerializer, messages).asDocument());
    }

    @Override
    public MongoCursor<Document> findEvents(MongoCollection collection, String aggregateType, String aggregateIdentifier,
                                            long firstSequenceNumber) {
        return collection.find(CommitEntry.forAggregate(aggregateType, aggregateIdentifier, firstSequenceNumber))
                         .sort(ascending(CommitEntry.SEQUENCE_NUMBER_PROPERTY)).iterator();
    }

    @Override
    public MongoCursor<Document> findEvents(MongoCollection<Document> collection, MongoCriteria criteria) {
        Bson sort = orderBy(ascending(CommitEntry.TIME_STAMP_PROPERTY, CommitEntry.SEQUENCE_NUMBER_PROPERTY));
    ...
    @Override
    public MongoCursor<Document> findLastSnapshot(MongoCollection collection, String aggregateType, String aggregateIdentifier) {
        return collection.find(and(
                eq(CommitEntry.AGGREGATE_IDENTIFIER_PROPERTY, aggregateIdentifier),
                eq(CommitEntry.AGGREGATE_TYPE_PROPERTY, aggregateType)
            )).sort(descending(CommitEntry.SEQUENCE_NUMBER_PROPERTY)).limit(1).iterator();
    }
    ...
    private static final class CommitEntry {
        ...
        private CommitEntry(Document dbObject) {
            ...
            List<Document> entries = (List<Document>) dbObject.get(EVENTS_PROPERTY);
        ...
        public Document asDocument() {
            List<Bson> events = new ArrayList<Bson>();

            for (EventEntry eventEntry : eventEntries) {
                events.add(eventEntry.asBson());
            }

            return new Document(AGGREGATE_IDENTIFIER_PROPERTY, aggregateIdentifier)
                    .append(SEQUENCE_NUMBER_PROPERTY, firstSequenceNumber)
                    .append(LAST_SEQUENCE_NUMBER_PROPERTY,
                         lastSequenceNumber)
                    .append(FIRST_SEQUENCE_NUMBER_PROPERTY,
                         firstSequenceNumber)
                    .append(TIME_STAMP_PROPERTY, firstTimestamp)
                    .append(FIRST_TIME_STAMP_PROPERTY, firstTimestamp)
                    .append(LAST_TIME_STAMP_PROPERTY, lastTimestamp)
                    .append(AGGREGATE_TYPE_PROPERTY, aggregateType)
                    .append(EVENTS_PROPERTY, events);
        }
        ...
        public static Bson forAggregate(String type, String aggregateIdentifier, long firstSequenceNumber) {
            return and(
                    eq(CommitEntry.AGGREGATE_IDENTIFIER_PROPERTY, aggregateIdentifier),
                    gte(CommitEntry.SEQUENCE_NUMBER_PROPERTY, firstSequenceNumber),
                    eq(CommitEntry.AGGREGATE_TYPE_PROPERTY, type)
            );
        }
    ...
    private static final class EventEntry {
        ...
        private EventEntry(Document document) {
            this.serializedPayload = document.get(SERIALIZED_PAYLOAD_PROPERTY);
            this.payloadType = (String) document.get(PAYLOAD_TYPE_PROPERTY);
            this.payloadRevision = (String) document.get(PAYLOAD_REVISION_PROPERTY);
            this.serializedMetaData = document.get(META_DATA_PROPERTY);
            this.eventIdentifier = (String) document.get(EVENT_IDENTIFIER_PROPERTY);
            this.sequenceNumber = (Long) document.get(EVENT_SEQUENCE_NUMBER_PROPERTY);
            this.timestamp = (String) document.get(EVENT_TIMESTAMP_PROPERTY);
        }

        public Class<?> getRepresentationType() {
            Class<?> representationType = String.class;
            if (serializedPayload instanceof Document) {
        ...
        public Document asBson() {
            return new Document(SERIALIZED_PAYLOAD_PROPERTY, serializedPayload)
                    .append(PAYLOAD_TYPE_PROPERTY, payloadType)
                    .append(PAYLOAD_REVISION_PROPERTY, payloadRevision)
                    .append(EVENT_TIMESTAMP_PROPERTY, timestamp)
                    .append(EVENT_SEQUENCE_NUMBER_PROPERTY, sequenceNumber)
