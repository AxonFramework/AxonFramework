...
public class MongoSagaRepository extends AbstractSagaRepository {
    ...
    protected Set<String> findAssociatedSagaIdentifiers(Class<? extends Saga> type, AssociationValue associationValue) {
        final Bson value = associationValueQuery(type, associationValue);

        MongoCursor<Document> dbCursor = mongoTemplate.sagaCollection().find(value).iterator();//.projection(elemMatch("sagaIdentifier")).iterator();
    ...
    private Bson associationValueQuery(Class<? extends Saga> type, AssociationValue associationValue) {
    ...
    protected void deleteSaga(Saga saga) {
        mongoTemplate.sagaCollection().findOneAndDelete(SagaEntry.queryByIdentifier(saga.getSagaIdentifier()));
    }
    ...
    protected void storeAssociationValue(AssociationValue associationValue, String sagaType, String sagaIdentifier) {
        mongoTemplate.sagaCollection().findOneAndUpdate(
                and(eq("sagaIdentifier", sagaIdentifier), eq("sagaType", sagaType)),
                new Document("$push",
                                  new Document("associations",
                                                    new Document("key", associationValue.getKey())
