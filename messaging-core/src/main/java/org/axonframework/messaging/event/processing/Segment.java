...
public class Segment implements Comparable<Segment> {
    ...
    protected Segment(int segmentId, int mask) {
        if (mask != 0 && mask + 1 != Integer.highestOneBit(mask + 1)) {
            throw new IllegalArgumentException("Invalid mask. It must end on a consecutive series of 1s");
        }
    ...
    public static Segment[] computeSegments(int... segments) {
        ...
        computeSegments(ROOT_SEGMENT, stream(segments).boxed().toList(), resolvedSegments);
    ...
    public Segment mergedWith(Segment other) {
        if (!this.isMergeableWith(other)) {
            throw new IllegalArgumentException("Given " + other + " cannot be merged with " + this);
        }
     ...
     * The first entry contains the original {@code segmentId}, with the newly calculated mask. (Simple left shift,
     * adding a '1' as LSB).
