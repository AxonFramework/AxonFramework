...
public class LoggingErrorHandler implements ErrorHandler {
    ...
    public void onError(Exception exception, EventMessage<?> event, EventListener eventListener,
        ...
        logger.error(String.format(
                "EventListener [%s] registered with EventProcessor [%s] failed to handle an event of type [%s]. " +
                        "Continuing processing with next handler",
                eventListener, eventProcessor, event.getPayloadType().getName()), exception);
    }
}
