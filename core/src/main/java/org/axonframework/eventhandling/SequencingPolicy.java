 ...
 * tasks for this processor at any time. <li>{@link FullConcurrencyPolicy}: Allows each tasks to be handled
 * independently of any other tasks. Tasks processing will typically start in the same order the tasks were scheduled
 * in. <li>{@link SequentialPerAggregatePolicy}: Will force events (only supports Event Handling tasks) generated by the
