...
public class EmbeddedEventStore extends AbstractEventStore {
     ...
     *                        in the cache is newer than the last processed event of the event processor. Once removed
     *                        the processor will be independently fetching directly from the event storage engine until
     *                        it has caught up again. Event processors will not notice this change during tracking (i.e.
     *                        the stream is not closed when an event processor falls behind and is removed).
    ...
    public EmbeddedEventStore(EventStorageEngine storageEngine, MessageMonitor<? super EventMessage<?>> monitor,
        ...
        super(storageEngine, monitor, snapshotChooser);
