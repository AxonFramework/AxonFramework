...
public class JdbcSequencedDeadLetterQueue<E extends EventMessage> implements SyncSequencedDeadLetterQueue<E> {
     ...
     * {@link Builder#genericConverter(Converter) generic Converter} and
     * {@link Builder#eventConverter(EventConverter) EventConverter}.
     *
        ...
        try {
            executeUpdate(connection,
                          c -> statementFactory.enqueueStatement(
                                  c, processingGroup, sequenceId, letter, nextIndexForSequence(sequenceId), context
                          ),
    ...
    public boolean process(@Nonnull Predicate<DeadLetter<? extends E>> sequenceFilter,
        ...
        if (claimedLetter != null) {
            return processInitialAndSubsequent(claimedLetter, processingTask, context);
        }
    ...
    public boolean process(@Nonnull Function<DeadLetter<? extends E>, EnqueueDecision<E>> processingTask,
        ...
        if (iterator.hasNext()) {
            ...
            return processInitialAndSubsequent(deadLetter, processingTask, context);
        }
    ...
    public static class Builder<E extends EventMessage> {
         ...
         * {@link #genericConverter(Converter) generic Converter}, and
         * {@link #eventConverter(EventConverter) EventConverter} are used.
         *
         ...
         * configured {@link #genericConverter(Converter) generic Converter}, and
         * {@link #eventConverter(EventConverter) EventConverter} are used.
         *
        ...
        /**
         * Sets the {@link Converter} to (de)serialize the {@link TrackingToken} (if present) of the event in the
         ...
         *
         * @param genericConverter The converter to use.
         * @return The current Builder instance, for fluent interfacing.
         */
        ...
        /**
         * Sets the {@link EventConverter} to (de)serialize the events, metadata and diagnostics of the
         ...
         * {@link DeadLetterJdbcConverter} when either of them are not explicitly configured.
         *
         * @param eventConverter The converter to use.
         * @return The current Builder instance, for fluent interfacing.
         */
         ...
         * {@link #schema(DeadLetterSchema) schema}, {@link #genericConverter(Converter) generic Converter}, and
         * {@link #eventConverter(EventConverter) EventConverter}.
         *
