...
/**
 * A {@link ProcessingContext} implementation specifically designed for dead letter processing.
 * <p>
 * This implementation provides resource storage for the {@link DeadLetter} and its contained {@link EventMessage},
 * allowing parameter resolvers to access the dead letter during processing. It delegates component lookups to the
 * provided {@link ApplicationContext}.
 * <p>
 * Lifecycle operations are not supported as dead letter processing does not follow the standard processing lifecycle.
 *
 * @author Mateusz Nowak
 * @since 5.0.0
 */
@Internal
class DeadLetterProcessingContext implements ProcessingContext {
    ...
    /**
     * Constructs a {@link DeadLetterProcessingContext} for the given {@code deadLetter}.
     * <p>
     * Uses {@link EmptyApplicationContext} for component lookups.
     *
     * @param deadLetter The dead letter being processed.
     */
    DeadLetterProcessingContext(@Nonnull DeadLetter<? extends EventMessage> deadLetter) {
        this(deadLetter, EmptyApplicationContext.INSTANCE);
    }

    /**
     * Constructs a {@link DeadLetterProcessingContext} for the given {@code deadLetter} and
     * {@code applicationContext}.
     *
     * @param deadLetter         The dead letter being processed.
     * @param applicationContext The application context for component lookups.
     */
    DeadLetterProcessingContext(@Nonnull DeadLetter<? extends EventMessage> deadLetter,
                                @Nonnull ApplicationContext applicationContext) {
        this.applicationContext = applicationContext;
        Message.addToContext(this, deadLetter.message());
        putResource(DeadLetter.RESOURCE_KEY, deadLetter);
    }
