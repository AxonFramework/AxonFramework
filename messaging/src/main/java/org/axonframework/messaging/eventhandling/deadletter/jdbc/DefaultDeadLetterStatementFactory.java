 ...
 * {@code PreparedStatements}. Furthermore, it uses the configurable {@code genericConverter} to serialize
 * {@link TrackingToken TrackingTokens} from the {@link ProcessingContext}. Lastly, this factory uses the
 * {@code eventConverter} to serialize the {@link EventMessage#payload() event payload},
 * {@link EventMessage#metadata() Metadata}, and {@link DeadLetter#diagnostics() diagnostics} of any
 * {@code DeadLetter}.
 * <p>
 * This factory and the {@link DeadLetterJdbcConverter} must use the same {@link Converter Converters} and
...
public class DefaultDeadLetterStatementFactory<E extends EventMessage> implements DeadLetterStatementFactory<E> {
    ...
    private void setEventFields(PreparedStatement statement,
        ...
        statement.setString(fieldIndex.getAndIncrement(),
                            eventMessage.payload() != null ? eventMessage.payload().getClass().getName() : null);
        statement.setString(fieldIndex.getAndIncrement(), null); // payloadRevision not used in AF5
    ...
    private void setDomainEventFields(PreparedStatement statement,
        ...
        statement.setLong(fieldIndex.getAndIncrement(), sequenceNumber != null ? sequenceNumber : -1L);
    }
    ...
    private void setDeadLetterFields(PreparedStatement statement,
        ...
        byte[] serializedDiagnostics = eventConverter.convert(letter.diagnostics(), byte[].class);
    ...
    public PreparedStatement requeueStatement(@Nonnull Connection connection,
        ...
        byte[] serializedDiagnostics = eventConverter.convert(diagnostics, byte[].class);
    ...
    protected static class Builder<E extends EventMessage> {
        ...
        /**
         * Sets the {@link Converter} to serialize the {@link TrackingToken} from the {@link ProcessingContext} when
         * storing a dead letter to the database.
         *
         * @param genericConverter The converter to use for {@link TrackingToken TrackingTokens}.
         * @return The current Builder, for fluent interfacing.
         */
        ...
        /**
         * Sets the {@link EventConverter} to serialize the {@link EventMessage#payload() event payload},
         ...
         *
         * @param eventConverter The converter to use for event payloads, metadata, and diagnostics.
         * @return The current Builder, for fluent interfacing.
         */
