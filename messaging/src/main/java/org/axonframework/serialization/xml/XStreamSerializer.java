...
public class XStreamSerializer extends AbstractXStreamSerializer {
     ...
     * the {@link Converter} defaults to a {@link ChainingContentTypeConverter}. Lastly, the builder adds Axon types for
     * XStream's security settings by including {@code "org.axonframework.**} as a wildcard type. This can be disabled
     * with the {@link Builder#disableAxonTypeSecurity()} operation when required.
     * <p>
     ...
     * registered too. Lastly, if the provided Converter instance is of type ChainingContentTypeConverter, then the
     * {@link XStreamSerializer#registerConverters(ChainingContentTypeConverter)} function will be called. This will
     * register the {@link Dom4JToByteArrayConverter}, {@link InputStreamToDom4jConverter}, {@link XomToStringConverter}
     * and {@link InputStreamToXomConverter} to the Converter chain.
     *
    ...
    public static class Builder extends AbstractXStreamSerializer.Builder {
        ...
        public XStreamSerializer build() {
            if (xStream == null) {
                logger.warn(
                        "An unsecured XStream instance allowing all types is used. It is strongly recommended to set the security context yourself instead!",
