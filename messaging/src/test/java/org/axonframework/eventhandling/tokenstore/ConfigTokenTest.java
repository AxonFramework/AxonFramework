...
class ConfigTokenTest {
    ...
    @Test
    void sameThrowsUnsupportedOperationException() {
        Map<String, String> configMap = Collections.singletonMap("some-key", "some-value");
        ConfigToken token = new ConfigToken(configMap);

        assertThrows(UnsupportedOperationException.class, () -> token.same(token));
        assertThrows(UnsupportedOperationException.class, () -> token.same(new GlobalSequenceTrackingToken(0)));
        assertThrows(UnsupportedOperationException.class, () -> token.same(null));
    }
}