...
package org.axonframework.messaging.monitoring.interceptors;

import org.axonframework.messaging.core.MessageDispatchInterceptorChain;
import org.axonframework.messaging.core.unitofwork.ProcessingContext;
import org.axonframework.messaging.core.unitofwork.ProcessingLifecycle;
import org.axonframework.messaging.monitoring.MessageMonitor;
import org.axonframework.messaging.monitoring.interceptors.MonitoringSubscriptionQueryUpdateDispatchInterceptor;
...
class MonitoringSubscriptionQueryUpdateDispatchInterceptorTest {

    private final MessageMonitor.MonitorCallback callback = mock(MessageMonitor.MonitorCallback.class);
    private final MessageMonitor<? super SubscriptionQueryUpdateMessage> messageMonitor = m -> callback;
    private final ProcessingContext processingContext = mock(ProcessingContext.class);
    private final SubscriptionQueryUpdateMessage message = mock(SubscriptionQueryUpdateMessage.class);
    @SuppressWarnings("unchecked")
    private final MessageDispatchInterceptorChain<SubscriptionQueryUpdateMessage> interceptorChain = mock(MessageDispatchInterceptorChain.class);

    private final MonitoringSubscriptionQueryUpdateDispatchInterceptor testSubject = new MonitoringSubscriptionQueryUpdateDispatchInterceptor(messageMonitor);
