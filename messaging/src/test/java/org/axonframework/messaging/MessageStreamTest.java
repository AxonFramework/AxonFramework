...
public abstract class MessageStreamTest<M extends Message> {
    ...
    void shouldMapSingleEntry_asFlux() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(testSubject.map(entry -> entry.map(input -> out))))
    ...
    void shouldMapSingleEntry_asMono() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(testSubject.map(entry -> entry.map(input -> out))).singleOrEmpty())
                    .expectNextMatches(entry -> entry.message().equals(out))
                    .verifyComplete();
    }
    ...
    void shouldMapSingleMessage_asFlux() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(testSubject.mapMessage(input -> out)))
                    .expectNextMatches(entry -> entry.message().equals(out))
                    .verifyComplete();
    }
    ...
    void shouldMapSingleMessage_asMono() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(testSubject.mapMessage(input -> out)).singleOrEmpty())
                    .expectNextMatches(entry -> entry.message().equals(out))
                    .verifyComplete();
    }
    ...
    void shouldMapMultipleEntries_asFlux() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(testSubject.map(entry -> entry.map(input -> input == in1 ? out1 : out2))))
    ...
    void shouldMapMultipleMessages_asFlux() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(testSubject.mapMessage(input -> input == in1 ? out1 : out2)))
                    .expectNextMatches(entry -> entry.message().equals(out1))
                    .expectNextMatches(entry -> entry.message().equals(out2))
                    .verifyComplete();
    }
    ...
    void shouldNotCallMapperForEmptyStream_asMono_ExplicitlyDeclaredEmpty() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(testSubject).singleOrEmpty())
                    .verifyComplete();
        assertFalse(invoked.get(), "Mapper function should not be invoked for empty streams");
    }
    ...
    void shouldInvokeOnNextHandler_asFlux() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(testSubject.onNext(entry -> invoked.set(true))))
    ...
    void shouldContinueOnSecondStreamOnError_asFlux() {
        ...
        StepVerifier.create(
            MessageStreamUtils.asFlux(testSubject.onErrorContinue(error -> {
                assertEquals(expectedError, FutureUtils.unwrap(error));
                return onErrorStream;
            })
        ))
    ...
    void shouldMoveToConcatWithStream_asFlux() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(testSubject.concatWith(secondStream)))
                    .expectNextMatches(entry -> entry.message().equals(expectedFirst))
                    .expectNextMatches(entry -> entry.message().equals(expectedSecond))
                    .verifyComplete();
    }
    ...
    void shouldConcatWithMultipleStreamsSequentially() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(concatenated))
    ...
    void shouldInvokeCompletionCallback_asFlux() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(testSubject.whenComplete(() -> invoked.set(true))))
                    .verifyComplete();
        assertTrue(invoked.get());
    }
    ...
    void shouldInvokeCompletionCallback_asMono() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(testSubject.whenComplete(() -> invoked.set(true))).singleOrEmpty())
                    .expectNextCount(1)
                    .verifyComplete();
        assertTrue(invoked.get());
    }
    ...
    void shouldNotInvokeCompletionCallbackForFailedStream_asMono() {
        ...
        StepVerifier.create(
            MessageStreamUtils.asFlux(((MessageStream.Single<M>) testSubject).whenComplete(() -> invoked.set(true)))
                .singleOrEmpty()
        )
        .verifyErrorMatches(e -> e == oops);
    ...
    void shouldResultInFailedStreamWhenCompletionCallbackThrowsAnException_asFlux() {
        ...
        StepVerifier.create(
            MessageStreamUtils.asFlux(testSubject.whenComplete(() -> {
                throw expected;
            }))
        )
    ...
    void shouldNotChangeStreamContentWithWhenComplete() {
        ...
        StepVerifier.create(MessageStreamUtils.asFlux(withCallback))
