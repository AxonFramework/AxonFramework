...
class CoordinatorTest {
    ...
    @Test
    void claimListenersAreInvokedSequentially() {
        CompletableFuture<Void> firstClaimListenerHandle = new CompletableFuture<>();
        AtomicBoolean firstClaimListenerInvoked = new AtomicBoolean();
        AtomicBoolean secondClaimListenerInvoked = new AtomicBoolean();
        TrackingToken testToken = new GlobalSequenceTrackingToken(0);
        ...
        testSubject = Coordinator.builder()
                                 ...
                                 .initialToken(es -> es.firstToken(null).thenApply(ReplayToken::createReplayToken))
                                 .maxSegmentProvider(e -> SEGMENTS.size())
                                 .addSegmentChangeListener(SegmentChangeListener.onClaim(segment -> {
                                     firstClaimListenerInvoked.set(true);
                                     return firstClaimListenerHandle;
                                 }))
                                 .addSegmentChangeListener(SegmentChangeListener.onClaim(segment -> {
                                     secondClaimListenerInvoked.set(true);
                                     return emptyCompletedFuture();
                                 }))
                                 .build();

        MessageStream<EventMessage> testStream = MessageStream.fromIterable(List.of(), e -> Context.empty());
        when(workPackage.hasRemainingCapacity()).thenReturn(true);
        when(workPackage.isAbortTriggered()).thenReturn(false);
        when(tokenStore.fetchSegments(eq(PROCESSOR_NAME), any())).thenReturn(completedFuture(SEGMENTS));
        when(tokenStore.fetchAvailableSegments(eq(PROCESSOR_NAME), any()))
        ...
        when(messageSource.open(streamingFrom(testToken), null)).thenReturn(testStream);
        doAnswer(runTaskSync()).when(executorService).submit(any(Runnable.class));
        doReturn(mock(ScheduledFuture.class)).when(executorService).schedule(any(Runnable.class), anyLong(), any(TimeUnit.class));

        CompletableFuture<Void> startHandle = CompletableFuture.runAsync(testSubject::start);
        assertWithin(500, TimeUnit.MILLISECONDS, () -> assertTrue(firstClaimListenerInvoked.get()));
        assertFalse(secondClaimListenerInvoked.get());

        firstClaimListenerHandle.complete(null);

        assertWithin(500, TimeUnit.MILLISECONDS, () -> assertTrue(secondClaimListenerInvoked.get()));
        assertWithin(500, TimeUnit.MILLISECONDS, () -> assertDoesNotThrow(startHandle::join));
    }

    @Test
    void releaseListenersAreInvokedSequentially() {
        CompletableFuture<Void> firstReleaseListenerHandle = new CompletableFuture<>();
        AtomicBoolean firstReleaseListenerInvoked = new AtomicBoolean();
        AtomicBoolean secondReleaseListenerInvoked = new AtomicBoolean();
        TrackingToken testToken = new GlobalSequenceTrackingToken(0);
        ...
        testSubject = Coordinator.builder()
                                 ...
                                 .addSegmentChangeListener(SegmentChangeListener.onRelease(segment -> {
                                     firstReleaseListenerInvoked.set(true);
                                     return firstReleaseListenerHandle;
                                 }))
                                 .addSegmentChangeListener(SegmentChangeListener.onRelease(segment -> {
                                     secondReleaseListenerInvoked.set(true);
                                     return emptyCompletedFuture();
                                 }))
                                 .build();

        MessageStream<EventMessage> testStream = MessageStream.fromIterable(List.of(), e -> Context.empty());
        when(workPackage.segment()).thenReturn(SEGMENT_ONE);
        when(workPackage.hasRemainingCapacity()).thenReturn(true);
        when(workPackage.isAbortTriggered()).thenReturn(false);
        when(workPackage.abort(any())).thenReturn(emptyCompletedFuture());
        when(tokenStore.fetchSegments(eq(PROCESSOR_NAME), any())).thenReturn(completedFuture(SEGMENTS));
        when(tokenStore.fetchAvailableSegments(eq(PROCESSOR_NAME), any()))
                .thenReturn(completedFuture(Collections.singletonList(SEGMENT_ONE)));
        when(tokenStore.fetchToken(eq(PROCESSOR_NAME), eq(SEGMENT_ONE), any()))
                .thenReturn(completedFuture(testToken));
        when(tokenStore.releaseClaim(eq(PROCESSOR_NAME), eq(SEGMENT_ONE.getSegmentId()), any()))
        ...
        CompletableFuture<Void> stopHandle = CompletableFuture.runAsync(() -> joinAndUnwrap(testSubject.stop()));
        assertWithin(500, TimeUnit.MILLISECONDS, () -> assertTrue(firstReleaseListenerInvoked.get()));
        assertFalse(secondReleaseListenerInvoked.get());

        firstReleaseListenerHandle.complete(null);

        assertWithin(500, TimeUnit.MILLISECONDS, () -> assertTrue(secondReleaseListenerInvoked.get()));
        assertWithin(500, TimeUnit.MILLISECONDS, () -> assertDoesNotThrow(stopHandle::join));
    }
