...
class ProcessingContextBindableComponentConfigurationEnhancerTest {

    @Test
    void wrapsParameterResolverFactory() {
        DefaultComponentRegistry registry = new DefaultComponentRegistry();
        registry.disableEnhancerScanning();
        registry.registerEnhancer(new ProcessingContextBindableComponentConfigurationEnhancer());
        ParameterResolverFactoryUtils.registerToComponentRegistry(
                registry,
                config -> new ProcessingContextParameterResolverFactory()
        );
        ...
        Assertions.assertInstanceOf(ProcessingContextBindingParameterResolverFactory.class, parameterResolverFactory);
    }
}