...
- Implementing complex business rule validations
- Monitoring system states across multiple events
- Creating dashboards and reporting views
- Triggering actions when conditions are met across time

== Understanding the Administrator Notification Feature

image::EventModeling_CreateCourse_Stickies.png[]

The diagram shows a Event Modeling flow. The stateful handler will monitor multiple events to determine when all courses are fully booked.
Then if the condition is met, it will send a notification to the administrator.
