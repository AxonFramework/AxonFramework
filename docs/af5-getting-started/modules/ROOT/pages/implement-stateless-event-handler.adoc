...
<5> Custom token initialization for testing with `InMemoryEventStorageEngine` (this is a workaround for testing) - will not be needed in further Axon Framework milestones.
...
class WhenStudentSubscribedThenSendNotificationTest extends UniversityApplicationTest { // <1>
