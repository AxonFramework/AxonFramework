...
Similar to command handling components, sagas have a clearly defined interface: they only respond to events. On the other hand, sagas often have a notion of time and may interact with other components as part of their event handling process. Axon Framework's test support module provides the `AxonTestFixture` to help you write tests for sagas.

[NOTE]
====
The `SagaTestFixture` from Axon Framework 4 has been replaced by the `AxonTestFixture` in Axon Framework 5.
...
The following code sample shows an example of how the fixture can be used to test a saga:
...
// Create your configurer with the saga configured
MessagingConfigurer configurer = MessagingConfigurer.create()
    .configureSaga(InvoicingSaga.class)
...
// Test the saga
fixture.given()
       .event(new InvoiceCreatedEvent(invoiceId, customerId, amount))
       .when()
       .event(new PaymentReceivedEvent(invoiceId, amount))
       .then()
       .commands(new MarkInvoiceAsPaidCommand(invoiceId));
----

For time-based saga testing, you can use the execute method to manipulate time:

[source,java]
----
// TODO: Document time-based testing for sagas in AxonTestFixture
// The specific API for time manipulation may differ from Axon 4
----
