...
It is important to note that the `JubRunrEventScheduler`, `QuartzEventScheduler`, `AxonServerEventScheduler` and `DbSchedulerEventSerializer` all should use the xref:ROOT:conversion.adoc#different-converter-for-events[event Converter] to convert and convert from the scheduled event.
If the `Converter` used by the scheduler does not align with the `Seralizer` used by the event store, exceptional scenarios should be expected.
The Quartz implementation's `Converter` can be set by defining a different `EventJobDataBinder`, whereas the JobRunr, Axon Server and db-scheduler implementations allows defining the used `Converter` directly through the builder.
...
It needs to be configured with a JobRunr `JobScheduler`, an `EventBus` and a `Converter`.
...
The `DbSchedulerEventScheduler` needs to be configured with a db-scheduler `Scheduler`, an `EventBus`, and a `Converter`.
...
public EventScheduler eventScheduler(
        @Qualifier("eventSerializer") final Converter converter,
