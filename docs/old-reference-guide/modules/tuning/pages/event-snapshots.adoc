...
Do make sure the `Converter` instance you use (which defaults to the XStreamSerializer (removed in Axon 5)) is capable of converting your aggregate.
The XStreamSerializer (removed in Axon 5) requires you to use either a Hotspot JVM, or your aggregate must either have an accessible default constructor or implement the `Serializable` interface.
...
* *Cache data in-memory.*
 ...
 This approach prevents the need to (re)convert aggregates when storing to disk and even off-heap.
