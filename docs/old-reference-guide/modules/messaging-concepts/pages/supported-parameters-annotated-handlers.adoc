...
|type: `ConflictResolver` |Will resolve the configured `ConflictResolver` instance. See the xref:axon-framework-commands:modeling/conflict-resolution.adoc[Conflict Resolution] section for specifics on this topic.
...
|subtype of `ScopeDescriptor` |The scope descriptor is helpful when xref:deadlines:README.adoc[scheduling a deadline] through the `DeadlineManager`. Note that the `ScopeDescriptor` only makes sense from within the scope of an Aggregate or Saga.
...
|type: `DeadLetter<EventMessage<?>>` |Will have the current xref:events:event-processors/README.adoc#dead-letter-queue[dead letter] related to the processed event injected. Note that the inserted field is _nullable_ since there is no guarantee the event of the handler is a dead letter, yes or no.
