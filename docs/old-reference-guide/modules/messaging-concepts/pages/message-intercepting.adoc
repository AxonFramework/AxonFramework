...
They can also block the command by returning a failed message stream using `MessageStream.failed()`.
...
public class MyCommandDispatchInterceptor implements MessageDispatchInterceptor<CommandMessage<?>> {
    ...
    public MessageStream<?> interceptOnDispatch(CommandMessage<?> command,
                                                ...
                                                MessageDispatchInterceptorChain<CommandMessage<?>> interceptorChain) {
...
Dispatch interceptors can enrich messages with metadata before they are dispatched.
This is commonly used for adding correlation IDs, user context, tracing information, or timestamps.

Let's create a dispatch interceptor that adds a correlation ID to every command if one is not already present:
...
public class CorrelationIdDispatchInterceptor implements MessageDispatchInterceptor<CommandMessage<?>> {
    ...
    public MessageStream<?> interceptOnDispatch(CommandMessage<?> command,
        ...
        if (!command.metadata().containsKey("correlationId")) {
            // Add correlation ID to metadata
            CommandMessage<?> enrichedCommand = command.withMetadata(
                Metadata.from(command.metadata())
                        .with("correlationId", UUID.randomUUID().toString())
            );
            return interceptorChain.proceed(enrichedCommand, context);
        }
        return interceptorChain.proceed(command, context);
    }
}
...
You can also use the `mapMessage()` method on `MessageStream` to modify the message after it has been processed by subsequent interceptors:

[source,java]
----
import org.axonframework.messaging.core.MessageStream;

public class TimestampDispatchInterceptor implements MessageDispatchInterceptor<CommandMessage<?>> {

    @Override
    public MessageStream<?> interceptOnDispatch(CommandMessage<?> command,
                                                ProcessingContext context,
                                                MessageDispatchInterceptorChain<CommandMessage<?>> interceptorChain) {
        // Proceed first, then add timestamp to the result
        return interceptorChain.proceed(command, context)
                               .mapMessage(msg -> ((CommandMessage<?>) msg).withMetadata(
                                   Metadata.from(msg.metadata())
                                           .with("dispatchedAt", Instant.now().toString())
                               ));
    }
}
----
...
Beyond structural validation, you can also use dispatch interceptors to enforce security policies or business rules.
Instead of throwing exceptions, use `MessageStream.failed()` to block messages:
...
public class SecurityDispatchInterceptor implements MessageDispatchInterceptor<CommandMessage<?>> {
    ...
    public MessageStream<?> interceptOnDispatch(CommandMessage<?> command,
        ...
        if (userId == null || userId.isEmpty()) {
            return MessageStream.failed(
                new SecurityException("User ID required in command metadata")
            );
        }

        // Could also check permissions here
        if (!hasPermission(userId, command)) {
            return MessageStream.failed(
                new SecurityException("User does not have permission to execute this command")
            );
        }
    ...
    private boolean hasPermission(String userId, CommandMessage<?> command) {
        // Check permissions based on user and command type
...
The `MessageHandlerInterceptorChain` is used to continue the dispatching process by calling `proceed()`.
The `ProcessingContext` provides the possibility to tie in logic prior, during or after (command) message handling (see xref:unit-of-work.adoc[Processing Lifecycle and Context] for more information about the phases).
...
If the `userId` is not present in the metadata, a failed stream will be returned which will prevent the command from being handled.
...
In the example below we can see a `@CommandHandlerInterceptor` annotated method which prevents command execution if a command's `state` field does not match the Entity's `state` field:
...
public class GiftCard {
    ...
    public MessageStream<?> intercept(RedeemCardCommand command,
                                      ProcessingContext context,
                                      MessageHandlerInterceptorChain interceptorChain) {
        if (this.state.equals(command.state())) {
            return interceptorChain.proceed(command, context);
        }
        return MessageStream.failed(new IllegalStateException("Invalid state"));
    }
}
----
...
public class EventLoggingDispatchInterceptor
    ...
    @Override
    public MessageStream<?> interceptOnDispatch(EventMessage<?> event,
                                                ProcessingContext context,
...
Similar to command dispatch interceptors, event dispatch interceptors can also enrich event metadata with user context, tracing information, or other cross-cutting concerns:
...
public class UserContextEventDispatchInterceptor implements MessageDispatchInterceptor<EventMessage<?>> {

    private final UserContextProvider userContextProvider;

    public UserContextEventDispatchInterceptor(UserContextProvider userContextProvider) {
    ...
    public MessageStream<?> interceptOnDispatch(EventMessage<?> event,
        ...
        EventMessage<?> enrichedEvent = event.withMetadata(
            Metadata.from(event.metadata())
                    .with("userId", userId)
                    .with("tenantId", tenantId)
        );

        return interceptorChain.proceed(enrichedEvent, context);
    }
}
----
...
Interceptors must implement the `MessageHandlerInterceptor` interface.
The `MessageHandlerInterceptorChain` is used to continue the dispatching process by calling `proceed()`.
The `ProcessingContext` provides the possibility to tie in logic prior, during or after (event) message handling (see xref:unit-of-work.adoc[Processing Lifecycle and Context] for more information about the phases).

Unlike dispatch interceptors, handler interceptors are invoked in the context of the event handler.
That means they can attach correlation data based on the message being handled to the processing context, for example.
This correlation data will then be attached to event messages being created in the context of that processing.
...
public class MyEventHandlerInterceptor
    ...
    public MessageStream<?> interceptOnHandle(EventMessage<?> event,
        ...
        if (userId == null) {
            return MessageStream.failed(new IllegalEventException("userId not present in metadata"));
        }
        if ("axonUser".equals(userId)) {
            return interceptorChain.proceed(event, context);
        }
        return MessageStream.failed(new IllegalEventException("User not authorized"));
    }
}
----
...
Interceptors must implement the `MessageHandlerInterceptor` interface.
The `MessageHandlerInterceptorChain` is used to continue the dispatching process by calling `proceed()`.
The `ProcessingContext` provides the possibility to tie in logic prior, during or after (query) message handling (see xref:unit-of-work.adoc[Processing Lifecycle and Context] for more information about the phases).

Unlike dispatch interceptors, handler interceptors are invoked in the context of the query handler.
That means they can attach correlation data based on the message being handled to the processing context, for example.
This correlation data will then be attached to messages being created in the context of that processing.
...
public class CardSummaryProjection {
    ...
    public MessageStream<?> intercept(QueryMessage<?, ?> queryMessage,
                                      ProcessingContext context,
                                      MessageHandlerInterceptorChain interceptorChain) {
        // Add your intercepting logic before
        MessageStream<?> result = interceptorChain.proceed(queryMessage, context);
...
*`@MessageHandlerInterceptor` method hierarchy on an Entity Member and Entity Root*
...
====
As explained above, `@MessageHandlerInterceptor` follow the entity hierarchy when applicable.
More concrete, if a parent entity has an interceptor, as well as one or several of the child entities, the interceptor chain will "move up" the hierarchy.
...
This is a clear discrepancy with all other message handler annotated methods that we cannot easily adjust.
As such, it is not recommended to rely on an invocation hierarchy for `@MessageHandlerInterceptor` annotated methods while sourcing an entity.
====
