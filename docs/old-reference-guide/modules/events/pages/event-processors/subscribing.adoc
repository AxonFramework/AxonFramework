...
1. The conversion type for the events.
...
If the conversion type is `JSON` this must be a `JsonPath` expression.
...
In this case add the conversion type, payload type, expression and fallback policy parameters for the alternative payload type.
