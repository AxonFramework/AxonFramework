...
* `MongoSequencedDeadLetterQueue` - Mongo variant of the dead-letter queue, available via the xref:mongodb-extension-reference::index.adoc[Mongo Extension]. It constructs a `deadletters` collection where it persists failed-events in.
