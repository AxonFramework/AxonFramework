...
You can retrieve a `SequencedDeadLetterProcessor` from the `LegacyEventProcessingConfiguration` based on a processing group name _if_ you have configured a dead-letter queue for this processing group.
