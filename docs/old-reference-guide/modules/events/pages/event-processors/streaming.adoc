...
Due to this, the "Pooled Streaming Event Processor" is the default Event Processor.
...
As the Event Store is a type of `StreamableMessageSource`, the default will be the Pooled Streaming Event Processor.
...
The Pooled Streaming Event Processor (PSEP) is the primary implementation of Streaming Processor in Axon Framework 5.
It provides enhanced performance and better resource utilization compared to previous implementations.

The PSEP supports all standard streaming operations:
...
=== Configuring a pooled streaming event processor

The Pooled Streaming Event Processor is the default and recommended streaming processor implementation.

// TODO: Add configuration examples for PooledStreamingEventProcessor in Axon 5

[NOTE]
====
The `TrackingEventProcessor` from Axon Framework 4 has been removed in Axon Framework 5.
The `PooledStreamingEventProcessor` is now the default and recommended streaming event processor, providing enhanced performance and better resource utilization.
====
