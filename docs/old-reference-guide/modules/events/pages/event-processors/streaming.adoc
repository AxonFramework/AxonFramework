...
Note that you can configure the token store to use for a streaming processor in the `LegacyEventProcessingConfigurer`:
...
Alternatively, inject the `LegacyEventProcessingConfigurer`, which allows more fine-grained customization:
...
If none of the other two options are available, you can adjust the number of segments through the Axon Framework API. The `StreamingEventProcessor` exposes the `splitSegment(int segmentId)` and `mergeSegment(int segmentId)` methods. To obtain the `StreamingEventProcessor`, you can use the `LegacyEventProcessingConfiguration` to retrieve the processor by name.
...
Assuming a `buildMultiStreamableMessageSource(...)` method is present, we can use the outcome to register a processor with the configuring `LegacyEventProcessingConfigurer`:
