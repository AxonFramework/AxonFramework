...
Whenever the xref:event-processors/index.adoc#error-handling[error handler] rethrows an exception (keep in mind, that by default the exception is just logged), a `TrackingEventProcessor` will retry processing the event using an incremental back-off period.
