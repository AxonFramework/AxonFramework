...
All three implementations support subscribing and tracking processors (see xref:event-processors/README.adoc[]).
...
To distribute events between bounded contexts, you can use Axon Server's xref:axon_server_ref:ROOT:axon-server/administration/multi-context.adoc[] solution, for example.
...
Axon provides a good bunch of these as extension modules, for example xref:amqp-extension-reference::index.adoc[Spring AMQP] or  xref:kafka_extension_guide::index.adoc[Kafka].
