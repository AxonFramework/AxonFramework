...
<persistence xmlns="http://java.sun.com/xml/ns/persistence" version="1.0">
    <persistence-unit name="eventStore" transaction-type="RESOURCE_LOCAL"> <1>
        ...
        <class>org.axonframework.messaging.jpa.eventstore.eventsourcing.SnapshotEventEntry</class>
    </persistence-unit>
</persistence>
----
...
public class AxonConfig {
    ...
    public EventStorageEngine eventStorageEngine(
            ...
            @Qualifier("eventSerializer") Converter eventSerializer,
            EntityManagerProvider entityManagerProvider,
            TransactionManager transactionManager
    ) {
         return JpaEventStorageEngine
             .builder()
             .snapshotSerializer(converter)
             .persistenceExceptionResolver(persistenceExceptionResolver)
    ...
    public void configureJdbcEventStorage(
        ...
        configurer.configureEmbeddedEventStore(config -> {
            JdbcEventStorageEngine storageEngine = JdbcEventStorageEngine
                .builder()
                .snapshotSerializer(config.converter())
                .connectionProvider(connectionProvider)
...
Please note that there are several other optional configuration properties, like the converters and an (optional) upcaster chain.
...
=== Influencing the conversion process

Event stores need a way to convert the event to prepare it for storage.
By default, Axon uses the XStreamSerializer (removed in Axon 5), which uses http://x-stream.github.io/[XStream] to convert events into XML.
XStream is reasonably fast and is more flexible than Java Conversion.
Furthermore, the result of XStream conversion is human-readable.
This makes it quite useful for logging and debugging purposes.

The XStreamSerializer (removed in Axon 5) can be configured.
...
Alternatively, Axon also provides the `JacksonConverter`, which uses https://github.com/FasterXML/jackson[Jackson] to convert events into JSON.
...
You may also implement your own converter, simply by creating a class that implements `Converter`, and configuring the event store to use that implementation instead of the default.
...
----
// Returns a Configurer instance with default components configured.
// We explicitly set `JacksonConverter` as desired event converter.
Configurer configurer = DefaultConfigurer.defaultConfiguration()
      .configureEventSerializer(c -> JacksonConverter.builder().build());
----
--
...
Alternatively, you can explicitly define your Converter in the Spring context:
...
@Bean
public Converter eventSerializer() {
    return JacksonConverter.builder().build();
}
----
--
====
...
It is possible to use a different converter for the storage of events, than all other objects that Axon needs to convert (such as commands, snapshots, sagas, etc).
While the XStreamSerializer (removed in Axon 5)'s capability to convert virtually anything makes it a  decent default, its output is not always a form that makes it nice to share with other applications. The `JacksonConverter` creates much nicer output, but requires a certain structure in the objects to convert.
This structure is typically present in events, making it a suitable event converter.

If no explicit `eventSerializer` is configured, events are serialized using the main converter that has been configured (which defaults to the XStreamSerializer (removed in Axon 5)).
