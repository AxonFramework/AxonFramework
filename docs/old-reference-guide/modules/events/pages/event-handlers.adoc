...
Event handling Components are defined using an `LegacyEventProcessingConfigurer`, which can be accessed from the global Axon `Configurer`.
`LegacyEventProcessingConfigurer` is used to configure an `LegacyEventProcessingConfiguration`.
Typically, an application will have a single `LegacyEventProcessingConfiguration` defined, but larger, more modular applications may define one per module.
