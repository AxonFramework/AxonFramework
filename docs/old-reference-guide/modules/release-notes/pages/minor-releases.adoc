...
* Allow usage of specific converters for the JpaDLQ https://github.com/AxonFramework/AxonFramework/pull/2486[#2486]
...
* DeadLetterQueue uses wrong Converter to (de)convert Tokens https://github.com/AxonFramework/AxonFramework/issues/2485[#2485]
...
* [#2431] Use `XStreamSerializer (removed in Axon 5)#defaultSerializer` to mitigate XStream exclusion issues https://github.com/AxonFramework/AxonFramework/pull/2434[#2434]
...
* Snapshots are not read with snapshot filter and same converter for events and snapshots https://github.com/AxonFramework/AxonFramework/pull/2286[#2286]
...
* Firstly, we updated the XStream version to 1.4.18. This upgrade was a requirement since several https://x-stream.github.io/changes.html[CVE's] were noted for XStream version 1.4.17.
 ...
 This change also has an impact on Axon Framework since the XStreamSerializer (removed in Axon 5) is the default converter.
 So as of this release, any usages of the default XStreamSerializer (removed in Axon 5) will come with warnings, stating it is highly recommended to use an `XStream` instance for which the security context is set through types or wildcards.
...
* A certain path within the `AxonServerEventStore` allowed for event retrieval without correctly converting from the `MetaData` of the events.
 If someone tried to access the `MetaData`, a `CannotConvertBetweenTypesException` was being thrown.
 This problem, among others, was remedied in pull request https://github.com/AxonFramework/AxonFramework/pull/1612[#1612], by ensuring the correct `Converter` taking gRPC message types into account is consistently used. 
...
* A conversion issue was found when working with the `ConfigToken` and de-/convert it through the `JacksonConverter`.
...
This has been resolved by using a `Supplier` of a `Converter` in the Builders instead, as described under https://github.com/AxonFramework/AxonFramework/issues/1054[this] issue.
