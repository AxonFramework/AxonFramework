...
- Support Apache Avro as message conversion format link:https://github.com/AxonFramework/AxonFramework/issues/3159[#3159]
...
- [#3159] `AvroConverter` - Documentation, code sample, and auto-config fix link:https://github.com/AxonFramework/AxonFramework/pull/3275[#3275]
...
* Detect empty snapshots due to Converter misconfiguration https://github.com/AxonFramework/AxonFramework/pull/2817[#2817]
...
* [#1645] Introduce ObjectNode-to/from-JsonNode `ContentTypeConverter` for the `JacksonConverter` https://github.com/AxonFramework/AxonFramework/pull/1995[#1995]
...
* Include message identifier in error message if de-conversion fails https://github.com/AxonFramework/AxonFramework/pull/2330[#2330]
...
* Removed deprecated code by updating the default converter initialization https://github.com/AxonFramework/AxonFramework/pull/2206[#2206]
...
* Wire the event `Converter` into `QuartzEventSchedulerFactoryBean` https://github.com/AxonFramework/AxonFramework/issues/2088[#2088]
...
* Aggregate Polymorphism has been introduced, allowing for an aggregate hierarchy to come naturally from a domain model.
 ...
 For more details on this feature, read up on it xref:4.12@axon-framework-reference:axon-framework-commands:modeling/aggregate-polymorphism.adoc[here].
...
* Both the XStreamSerializer (removed in Axon 5) and `JacksonConverter` provide a means to toggle on "lenient conversion" through their builders.
...
* The `ScopeDescriptor` used by the `DeadlineManager` had conversion issue when a user would migrate from an Axon 3.x application to Axon 4.x.
