...
====
For command routing in distributed environments (for example, through Axon Server), use the `@RoutingKey` annotation on the command field that should be used for routing.
...
This allows you to perform operations that should only occur when handling newly generated events (for example, sending external notifications).
