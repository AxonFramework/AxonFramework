...
* `SerializerThreadCount` - the number of threads to use to pre-convert events.
 This defaults to `1`, but is ignored if no converter is configured.
* `Converter` - the converter to perform pre-conversion with.
 When a converter is configured, the `DisruptorCommandBus` will wrap all generated events in a `SerializationAware` message.
