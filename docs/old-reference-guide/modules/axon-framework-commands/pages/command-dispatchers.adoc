...
Use `ProcessingContext.empty()` when dispatching from outside a message handling context (for example, from a REST endpoint).
...
* *Use `CommandGateway`* - When dispatching commands from outside a message handling context (for example, REST endpoints)
