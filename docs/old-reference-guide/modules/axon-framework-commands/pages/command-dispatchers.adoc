...
public class GiftCard {
    ...
    @CommandHandler
    public void handle(RedeemCardCommand command, CommandDispatcher dispatcher) { // <1>
...
The framework has built-in support for this: when a `@CommandHandler` annotated constructor executes successfully, the value of the `@AggregateIdentifier` annotated field is automatically returned.
...
The `ProcessingContext` is a key concept in Axon 5 that replaces the thread-local `UnitOfWork` from Axon 4.
