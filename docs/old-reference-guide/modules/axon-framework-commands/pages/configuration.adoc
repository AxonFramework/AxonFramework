...
Core concepts within the Command Model are the xref:modeling/command-model.adoc[Aggregates] that are implemented.
...
Often the command handler functions are placed directly on the xref:modeling/command-model.adoc[aggregate]. When this approach is taken, simply registering the Aggregate as described above is sufficient for all its command handler methods to be registered too.
