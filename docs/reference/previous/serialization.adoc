...
A flexible, general-purpose serializer like http://x-stream.github.io/[XStream] is quite ideal for this purpose.
...
XStream is very configurable and extensible.
...
But if XStream encounters an XML element called `"clientId"`, it is a known alias and will be resolved to field name `"customerId"`.
...
For more information on how to configure your `EventSerializer` to something different, check out the documentation about <<Serialization>>.
...
An link:events/event-versioning.adoc#_event_upcasting[upcaster] works on a given content type (e.g. dom4j Document).
