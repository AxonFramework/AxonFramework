...
Since an link:../events[Event] message is unidirectional, handling an event does not include any return values.
...
This behaviour could be supported generically by implementing link:message-intercepting.md[interceptors] which perform exception wrapping for you.
...
More specifically, this is a type of link:message-intercepting.md[message handler interceptor] dedicated to reacting to exceptional results.
Note that such an `@ExceptionHandler` will only handle exceptions thrown from message handling functions in the same class.
For more specifics on how to use this annotation, check out link:message-intercepting.md#exceptionhandler[this] section.