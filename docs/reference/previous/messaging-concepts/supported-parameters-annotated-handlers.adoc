...
|type: `TrackingToken` |Will have the current link:../events/event-processors/streaming.adoc#_tracking_tokens[token] related to the processed event injected. Note that this will only work for `StreamingEventProcessor` instances, as otherwise, there is no token attached to the events.
...
|type: `DeadLetter<EventMessage<?>>` |Will have the current link:../events/event-processors/README.adoc#_dead_letter_queue[dead letter] related to the processed event injected. Note that the inserted field is _nullable_ since there is no guarantee the event of the handler is a dead letter, yes or no.
