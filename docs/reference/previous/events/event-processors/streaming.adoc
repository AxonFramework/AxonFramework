...
The `StreamingEventProcessor`, or Streaming Processor for short, is a type of link:README.adoc[Event Processor].
...
An example of a `StreamableMessageSource` is the link:../event-bus-and-event-store.adoc#_event_store[`EventStore`], like for example link:../../../axon-server-introduction.md[Axon Server] or an link:../event-bus-and-event-store.adoc#_embedded_event_store[RDBMS].
...
The Streaming Processors have several additional components that you can configure, next to the link:README.adoc#_general_processor_configuration[base options].
...
Whenever the link:README.adoc#_event_processor__error_handler[error handler] rethrows an exception, a `TrackingEventProcessor` will retry processing the event using an incremental back-off period.
...
This support is mentioned in the link:../../messaging-concepts/supported-parameters-annotated-handlers.adoc#_supported_parameters_for_event_handlers[Supported Parameters for Event Handlers] section.
...
For that, link:../../monitoring/processors.adoc#_event_tracker_status[status of the processor] is used.
