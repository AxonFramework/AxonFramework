...
The Processing Group provides a level of configurable non-functional requirements, like <<Processing Group - Listener Invocation Error Handler,error handling>> and the link:streaming.adoc#_sequential_processing[sequencing policy].
...
The (default) event processor used can be adjusted, as is shown in the link:subscribing.adoc#_configuring[subscribing] and link:streaming.adoc#_configuring[streaming] sections.
...
link:../event-handlers.adoc#_registering_event_handlers[This] section describes the process to register an event handler, whereas link:../../sagas/implementation.adoc#_configuring_a_saga[this] page describes the saga registration process.
...
To order event handlers within an Event Processor, the order in which event handlers are registered (as described in the link:../event-handlers.adoc#_registering_event_handlers[Registering Event Handlers] section) is guiding.
...
When an exception is thrown when a processor is trying to commit a transaction, update a link:streaming.adoc#_token_store[token], or in any other part of the process, the exception will be propagated.

In the case of a link:streaming.adoc#_error_mode[Streaming Event Processor], this means the processor will go into error mode, releasing any tokens and retrying at an incremental interval (starting at 1 second, up to max 60 seconds).
A link:subscribing.adoc#_error_mode[Subscribing Event Processor] will report a publication error to the component that provided the event.
...
The behaviour for the Subscribing- and the Streaming Event Processor can respectively be found link:subscribing.adoc#_error_mode[here] and link:streaming.adoc#_error_mode[here].
...
When you only log the error and allow processing to proceed, you will most likely end up with missing data until you fix the predicament and link:streaming.adoc#_replaying_events[replay]
past events.
...
An essential concept of Axon Frameworks event processors is the maintenance of event ordering, even when you configure link:streaming.adoc#_parallel_processing[parallel processing].
...
It does so by maintaining a sequence identifier for each event, determined by the link:streaming.adoc#_sequential_processing[sequencing policy].

____

*Is there support for Sagas?*

Currently, there is _no_ support for using a dead-letter queue for link:../../sagas/README.adoc[sagas].
...
For link:subscribing.adoc#_configuring[Subscribing] and link:streaming.adoc#_configuring[Streaming] Event Processor specific options, their respective sections should be checked.
...
Differently put, an link:../../messaging-concepts/message-intercepting.adoc#_event_handler_interceptors[EventHandlerInterceptor] can be registered to Event Processors.
