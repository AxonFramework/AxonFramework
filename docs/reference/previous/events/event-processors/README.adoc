...
link:../event-handlers.md[Event handlers] define the business logic to be performed when an event is received.
_Event Processors_ are the components that take care of the technical aspects of that processing.
They start a link:../../messaging-concepts/unit-of-work.md[unit of work] and possibly a transaction.
However, they also ensure that link:../../messaging-concepts/message-correlation.md[correlation data] can be correctly attached to all messages created during event processing, among other non-functional requirements.

The image below depicts a representation of the organization of Event Processors and Event Handlers:

image:../../../.gitbook/assets/event-processors.png[Organization of Event Processors and Event Handlers]
...
The Processing Group provides a level of configurable non-functional requirements, like <<processing-group-listener-invocation-error-handler,error handling>> and the link:streaming.md#sequential-processing[sequencing policy].
...
Event Processors come in roughly two forms: link:subscribing.md[Subscribing] and link:streaming.md[Streaming].
...
For more specifics on either type, consult their respective sections link:subscribing.md[here] and link:streaming.md[here].
...
Furthermore, the default processor implementation used by Axon is the link:streaming.md[Tracking Event Processor].
The (default) event processor used can be adjusted, as is shown in the link:subscribing.md#configuring[subscribing] and link:streaming.md#configuring[streaming] sections.

Event handlers, or Event Handling Components, come in roughly two flavors: "regular" (singleton, stateless) event handlers and link:../../sagas/README.md[sagas].
link:../event-handlers.md#registering-event-handlers[This] section describes the process to register an event handler, whereas link:../../sagas/implementation.md#configuring-a-saga[this] page describes the saga registration process.
...
This annotation resembles the Processing Group level discussed in the link:README.md#event-processors[introduction].
...
To order event handlers within an Event Processor, the order in which event handlers are registered (as described in the link:../event-handlers.md#registering-event-handlers[Registering Event Handlers] section) is guiding.
...
When an exception is thrown when a processor is trying to commit a transaction, update a link:streaming.md#token-store[token], or in any other part of the process, the exception will be propagated.

In the case of a link:streaming.md#error-mode[Streaming Event Processor], this means the processor will go into error mode, releasing any tokens and retrying at an incremental interval (starting at 1 second, up to max 60 seconds).
A link:subscribing.md#error-mode[Subscribing Event Processor] will report a publication error to the component that provided the event.
...
The behaviour for the Subscribing- and the Streaming Event Processor can respectively be found link:subscribing.md#error-mode[here] and link:streaming.md#error-mode[here].
...
When you only log the error and allow processing to proceed, you will most likely end up with missing data until you fix the predicament and link:streaming.md#replaying-events[replay]
past events.
...
An essential concept of Axon Frameworks event processors is the maintenance of event ordering, even when you configure link:streaming.md#parallel-processing[parallel processing].
...
It does so by maintaining a sequence identifier for each event, determined by the link:/axon-framework/events/event-processors/streaming.md#sequential-processing[sequencing policy].

____

*Is there support for Sagas?*

Currently, there is _no_ support for using a dead-letter queue for link:/axon-framework/sagas/README.md[sagas].
...
For link:subscribing.md#configuring[Subscribing] and link:streaming.md#configuring[Streaming] Event Processor specific options, their respective sections should be checked.
...
Since the Event Processor is the invoker of event handling methods, it is a spot to configure link:../../messaging-concepts/message-intercepting.md[Message Handler Interceptors] too.
...
Differently put, an link:../../messaging-concepts/message-intercepting.md#event-handler-interceptors[EventHandlerInterceptor] can be registered to Event Processors.
...
Message Monitors (discussed in more detail link:../../monitoring/metrics.md[here]) allow for monitoring the flow of messages throughout an Axon application.
...
Axon uses the `TransactionManager` to attach a transaction to every link:../../messaging-concepts/unit-of-work.md[Unit of Work].
...
It is the `RollbackConfiguration` that decide when a link:../../messaging-concepts/unit-of-work.md[Unit of Work] should rollback the transaction.
The default `RollbackConfiguration` is to rollback on any type of `Throwable`; the link:../../messaging-concepts/unit-of-work.md[Unit of Work] page describes the other options you can choose.
