...
Operations like link:streaming.adoc#_replaying_events[replaying] are, therefore, not an option for any Subscribing Processor as long as the `SubscribableMessageSource` follows this paradigm.
...
Instead, the "Tracking Event Processor" (a link:streaming.adoc#_streaming_event_processor[Streaming Processor] implementation) takes up that role.
...
Other than configuring that an app uses a Subscribing Event Processor, everything is covered link:README.adoc#_general_processor_configuration[here].
...
Whenever the link:README.adoc#_event_processor__error_handler[error handler] rethrows an exception, the `SubscribingEventProcessor` will have it bubble up to the publishing component of the event.
