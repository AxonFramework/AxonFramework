...
It aligns perfectly with the Event Modeling which we used to design our application and also with the Axon Framework architecture which is designed around messages.
...
[NOTE]
====
In Milestone 1, only the in-memory implementation of the Event Store is supported, but this still allows you to try out the new Command handling API.
====
