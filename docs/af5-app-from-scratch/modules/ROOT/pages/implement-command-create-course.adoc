...
Pay attention that `State` per feature/slice approach gives us a high level of encapsulation, because we may keep it package-private. The cohesion is also higher, because you don't care about unrelated to the current process properties. It reduces the cognitive load on a developer—you need to understand that the state needed for the particular slice.
