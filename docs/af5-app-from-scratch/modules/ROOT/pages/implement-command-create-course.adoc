...
class CreateCourseCommandHandler {
    ...
    @CommandHandler // <1>
     void handle(
        ...
        var event = new CourseCreated(command.courseId(), command.name(), command.capacity());  // <5>
...
<.> The `EventSink` interface requires an implementation of the `EventMessage` as a second argument.
