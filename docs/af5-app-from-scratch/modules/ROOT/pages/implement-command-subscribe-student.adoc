...
Whereas, for example `StudentEnrolledFaculty` - we care about just one event for the given `studentId`, other students are not interested while processing this command and there are no business rules between them.
With the subscription story is different, because of that we have a limit of students per course and also the limit of courses per student.
...
But in domain experts language the 'StudentSubscribed' it's just one fact, which influences rules around `Student` and `Course`.
As far as we're in the single bounded context, and we have all events in one storage, we good to go and define our custom Event Criteria to shape our `State` which mix properties for `Student` and `Course`!
The single responsibility of the `State` it's just to give us enough information to say if the command satisfies business rules.
...
<3> As you may see at the highest level those `EventCriteria` are combined with `either` method, but when we define tags in `withTags` it means that the certain type of event requires all of them.
...
So, we split `StudentSubscribed` and `StudentUnsubscribed` events into two separate criteria, because we need to load all events of those types for either `courseId` or `studentId`.
