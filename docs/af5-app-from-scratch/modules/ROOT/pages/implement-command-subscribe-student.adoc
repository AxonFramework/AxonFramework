...
**Rule #2: The student is not already subscribed to the course **
**Rule #3: The student is not subscribed to too many courses **

We're going to implement both rules and once.
What else do we need in the State object?
...
While implementing previous feature we defined that we want to build our state based on event's that are tagged with `courseId` by using this `@EventSourcedEntity(tagKey = "courseId")`.
For now it's not enough, because as you want we need to build our state based on `studentId` and `courseId` tags.
...
Whereas for example `StudentEnrolledFaculty` - we care about just one event for the given `studentId`, other students are not interested while processing this command.

As you may see here we're able to define the events we'd like to load dynamically.
It's where the Dynamic Consistency Boundary comes into play.
Before we had to load all events for the given aggregate, now we can load only those events that are relevant for the given command.
...
It gives us better accessibility of our system - thanks to that, as you see there is no `CourseRenamed` event in our criteria, so the Faculty administrator is still able to rename the course.
In case of Aggregates those operation may clash, or you need to introduce separated entity for such courses.
...
If you do just `EventCriteria.match().eventsOfTypes().withTags(...)` you load all events for the given tag, so there are more chances to clash with other changes, which cause events that do not influce this certain business rules.
