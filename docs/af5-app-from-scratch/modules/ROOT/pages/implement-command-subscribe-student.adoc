...
<7> This handler for `StudentUnsubscribed` is an exact opposite of the `evolve` method for `StudentSubscribed` event. We decreased the numbers which we increased in the previous one.
...
You may think about that in such a way: before event could have just one tag—the aggregateId, but now we can have multiple tags for the same event.
====
...
In case of Aggregates, those operations may clash, or you need to introduce separated entity for the name if you want to avoid concurrency access issues.
