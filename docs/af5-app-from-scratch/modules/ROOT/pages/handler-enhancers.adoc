...
Handler enhancers differ from xref:axon-framework-reference:messaging-concepts:message-intercepting.adoc[message handler interceptors] by the access they provide to the message handling component (for example, the aggregate member) at the resolution time.
...
In this case, indicating the handler is only suitable if the meta-data key matches a value.
