...
The other way is to add a `SequencedDeadLetterQueue` to a specific processing group in the `LegacyEventProcessingConfigurer` using the `registerDeadLetterQueue` method.
