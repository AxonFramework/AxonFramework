...
Explicitly creating the emitter using `QueryUpdateEmitter.forContext(context)` is only necessary when manually registering query handlers or when you need direct control over the emitter creation.
====
