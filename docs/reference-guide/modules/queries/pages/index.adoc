...
These are routed to a single handler, which is expected to return a single result. This is the most common type of query. When multiple handlers are available, one is selected based on the query bus implementation.
