...
* `namespace`: The bounded context or namespace for the query (defaults to the package name, or the value from a `@Namespace` annotation if present, as described xref:messaging-concepts:anatomy-message.adoc#namespace-annotation[here])
