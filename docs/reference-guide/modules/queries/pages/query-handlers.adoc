...
* `namespace`: The bounded context or namespace for the query.
When not defined on the `@Query` annotation, Axon searches for the `@Namespace` annotation (as described xref:messaging-concepts:anatomy-message.adoc#namespace-annotation[here]).
When `@Namespace` is also not present, this attribute defaults to the package name of the query class
...
For subscription queries, you can inject a `QueryUpdateEmitter` to an xref:messaging-concepts:supported-parameters-annotated-handlers.adoc#supported-parameters-for-event-handlers[event handler] to emit updates to subscribers as a projection changes or new data becomes available:
