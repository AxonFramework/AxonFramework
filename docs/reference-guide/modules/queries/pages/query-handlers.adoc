...
The handling of a query comes down to an annotated handler returning the query's response. The goal of this chapter is to describe what such a `@QueryHandler` annotated method looks like, as well as describing the call order and response type options. For configuration of query handlers and the `QueryBus`, it is recommended to read the xref:configuration.adoc[Configuration] section.
...
====
It's highly recommended to annotate all query message classes with `@Query`. This allows you to:
...
The query message's name is defined by the query payload class:

. If the `@Query` annotation is present on the query payload class, it defines the qualified name and version of the query.
. Otherwise, the fully qualified class name of the query payload class is used as the query name.
...
----
<1> The `queryName` attribute is **required** when the first parameter is not the `@Query` annotated payload class. Without it, Axon wouldn't know which query this handler should handle.
<2> The handler receives the query as a `Map`, allowing flexible access to the query data.

This approach allows the same query to be handled by different handlers using different Java representations, as long as each handler explicitly declares which query name it handles.
...
// Another handler could receive the same query in a different representation
...
====
Axon invokes exactly one query handler per query message. If multiple handlers are registered for the same query name, one will be selected (typically in round-robin fashion for distributed setups). To avoid ambiguity, ensure only one handler is registered per query name, or use explicit routing if multiple handlers are intentional.
====
...
The `ProcessingContext` is mandatory in query handlers and is always created by Axon when processing a query message. It provides access to resources, correlation data, and lifecycle hooks during query processing.
...
For subscription queries, you can inject a `QueryUpdateEmitter` to emit updates to subscribers. The emitter must be created from the `ProcessingContext` to ensure it's context-aware:
...
====
The `QueryUpdateEmitter` must be created using `QueryUpdateEmitter.forContext(ProcessingContext)` rather than being injected as a field. This ensures the emitter is aware of the current processing context and can properly handle correlation data and lifecycle hooks.
...
public class CardSummaryProjection {
    @EventHandler
    public void on(CardRedeemedEvent event, ProcessingContext context) {
        QueryUpdateEmitter emitter = QueryUpdateEmitter.forContext(context); // ✅ Correct!
        // Use emitter...
    }
}
----
