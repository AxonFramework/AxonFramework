...
MessagingConfigurer configurer = MessagingConfigurer.create()
    .registerQueryBus(config -> {
        UnitOfWorkFactory unitOfWorkFactory = config.getComponent(UnitOfWorkFactory.class);
