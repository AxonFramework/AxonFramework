...
[NOTE]
====
Axon Framework does not support scatter-gather queries where multiple handlers respond to the same query. Each query name can have only one registered handler.

If you need to collect information from multiple sources, consider using multiple separate queries or implementing an aggregating query handler that coordinates the data collection internally.
====
