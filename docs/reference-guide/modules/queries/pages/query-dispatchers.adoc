...
How to configure and specifics on the query gateway and bus implementations are discussed xref:implementations.adoc[here].
...
[IMPORTANT]
====
**Axon 5 removed scatter-gather query support.**

In Axon 4, it was possible to register multiple handlers for the same request-response combination and use scatter-gather queries to collect responses from all handlers.
This feature has been removed due to limited use.

If you rely on scatter-gather queries, please provide feedback at https://github.com/AxonFramework/AxonFramework/issues/3689[this GitHub issue].
====
...
[NOTE]
====
The `QueryBus` is a low-level infrastructure component.
...
When multiple handlers are available for the same query, the query is routed to one of them.

=== Query handler example

Here's a simple query handler:

[source,java]
----
@QueryHandler // <1>
public List<String> query(FetchItemsQuery query) {
    // return the query result based on given criteria
    return repository.findItems(query.getCriteria());
}
----

<1> By default, the name of the query is the fully qualified class name of the query payload (`FetchItemsQuery` in our case).
You can override this behavior by setting the `queryName` attribute of the `@QueryHandler` annotation.
...
[IMPORTANT]
.Subscription query API changes in Axon 5
====
Axon 5 has simplified the subscription query API:

* Returns a single `Publisher` that combines initial results and updates
* No more `ResponseTypes` - use `Class` types directly
* No need to explicitly close subscriptions - cancelling the `Publisher` subscription automatically closes the query
* `QueryUpdateEmitter` must be injected as a handler parameter (not as a field)
* `QueryUpdateEmitter` is now `ProcessingContext`-aware
====
...
[IMPORTANT]
.QueryUpdateEmitter injection in Axon 5
====
In Axon 5, the `QueryUpdateEmitter` must be injected as a handler parameter, not as a class field.
This makes it `ProcessingContext`-aware and ensures it's used correctly within message handling functions.

[source,java]
----
// ❌ WRONG - Don't inject as field
public class MyProjector {
    private final QueryUpdateEmitter emitter; // Don't do this

    @EventHandler
    public void on(MyEvent event) {
        emitter.emit(...); // This won't work correctly
    }
}

// ✅ CORRECT - Inject as parameter
public class MyProjector {
    @EventHandler
    public void on(MyEvent event, QueryUpdateEmitter emitter) {
        emitter.emit(...); // This works correctly
    }
}
----
====
...
QueryBus queryBus = configuration.component(QueryBus.class);
----
