...
====
Since subscription queries already require `reactor-core`, consider using the xref:axon-framework-reference:ROOT:modules.adoc#axon-reactor[Reactor extension]'s `ReactiveQueryGateway`.
It returns `Flux<R>` for subscription queries natively and provides reactive dispatch interceptors with access to Reactor context (for example, `ReactiveSecurityContextHolder` in Spring WebFlux applications).
====
...
.Reactive query gateway
====
Since streaming queries already require `reactor-core`, consider using the xref:axon-framework-reference:ROOT:modules.adoc#axon-reactor[Reactor extension]'s `ReactiveQueryGateway`.
It provides `streamingQuery()` returning `Flux<R>` directly, along with `query()` returning `Mono<R>` for point-to-point queries.
Reactive dispatch interceptors enable access to Reactor context at the gateway boundary.
====
