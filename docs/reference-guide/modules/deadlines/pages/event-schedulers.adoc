...
It is important to note that the `JubRunrEventScheduler`, `QuartzEventScheduler`, `AxonServerEventScheduler` and `DbSchedulerEventSerializer` all should use the xref:ROOT:conversion.adoc[event Converter] to serialize and deserialize the scheduled event.
