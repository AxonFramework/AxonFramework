...
Implementations like `JacksonConverter` and `AvroConverter`, as described in more detail in the <<converter_implementations,Converter implementations>> section, handle the actual conversion logic. We call this instance the general converter, is it is generally used for all non-`Message` components.
