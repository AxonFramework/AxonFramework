...
**JPA fallback**::
If Axon Server is not available or disabled, Axon will use JPA-based implementations for:
* Event storage (using a relational database)
* Token storage for event processors

**Extension-based implementations**::
Other extensions can provide additional implementations:
* MongoDB event store (via the Mongo extension)
* Kafka event distribution (via the Kafka extension)
* AMQP event distribution (via the AMQP extension)
