...
For easy configuration of Event Handling Components to Event Processor purely based on the Event Processor name, you can use the `@Namespace` annotation in combination with the `EventHandlerSelector.matchesNamespaceOnType()`.

The `@Namespace` annotation can be placed on the Event Handling Component, where the value of the annotation dictates the Event Processor name it belongs to. Then, when defining an `EventProcessorDefinition`, using the `EventHandlerSelector.matchesNamespaceOnType()` with the same name as defined in the `@Namespace` will automatically match handlers.
...
With `@Namespace` on the `OrderEventHandler`, the `OrderEventHandler` signals it **belongs** to the Event Processor named `orders`. Note that the `@Namespace` annotation can be placed on a class (as shown above), on an enclosing class, on a `package-info.java` file, and on a `module-info.java` file. The `@Namespace` annotation is searched for in the order derived above, thus: allows you to define a
...
These layers, for example, allow you to add the `@Namespace` annotation on the `package-info.java` file wherein the `OrderEventHandler` other event handlers are packaged.
...
These methods use the processor name as the namespace to match, making it easy to organize processors by namespace / bounded context.
====
