...
For more details on field filters, be sure to read the dedicated xref:field-filters.adoc[field filters] section.
...
Besides <<given_events,given events>>, you are able to derive the initial state based on command(s) as well.
...
Besides <<when_commands,when commands>>, you are able to trigger the test based on event(s) as well.
...
[#then_execute]
==== Then custom execute verification

For complex verification scenarios you can use `execute(Function<Configuration, ?>)`.
The `execute()` grants access to the entire `Configuration`, allowing users to access any component they require.
...
To that end, the list below shows some more complete tests for <<testing_command_handling_components,command handling components>>, <<testing_event_handling_components,event handling components>>, <<testing_event_sourced_entities,event-sourced entities>>, and testing <<testing_query_handling_components,query handling components>>.
...
Here is an example test scenario how an Event Handling Component sends emails as a result of handling an event:
...
In this test example, the developer(s) decided to combine the command handler with the event-sourced entity in one object.
