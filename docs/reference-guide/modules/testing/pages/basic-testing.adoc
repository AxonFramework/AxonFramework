...
* `excludeWhenPhaseMessages()` - Exclude the messages the `when` phase published or dispatched itself from the `then` phase recordings.

By default the `then` phase records every message that passed through the `EventSink` and the `CommandBus` during the `when` phase, including the one the `when` phase published on your behalf.
`excludeWhenPhaseMessages()` narrows `events(...)`, `eventsSatisfy(...)`, `eventsMatch(...)`, `noEvents()`, `commands(...)`, `commandsSatisfy(...)`, `commandsMatch(...)` and `noCommands()` to the output of the handlers under test.
Messages are excluded by identifier, so a handler republishing a payload equal to the `when` message is still recorded.
