...
IMPORTANT: Always call `fixture.stop()` in your `@AfterEach` method to properly release resources. You can use the xref:junit_extension[`AxonTestFixtureExtension`] with junit to automate this process.
...
[#junit_extension]
== JUnit `AxonTestFixtureExtension`

If you use junit for testing, you can simplify the lifecycle management of the `AxonTestFixture` by using the `AxonTestFixtureExtension`.
This extension automatically creates and stops the fixture for you, so you do not have to worry about that in your tests.

To use the extension, simply annotate your test class with `@ExtendWith(AxonTestFixtureExtension.class)` and declare a `AxonTestFixture` field in your test class. The extension will automatically inject the fixture into your test class and manage its lifecycle.

[source,java]
----
@ExtendWith(AxonTestFixtureExtension.class) <1>
class AccountEntityTest {

    @ProvideAxonTestFixture <2>
...
<1> The `@ExtendWith(AxonTestFixtureExtension.class)` annotation tells JUnit to use the `AxonTestFixtureExtension` for this test.
<2> The `@ProvideAxonTestFixture` annotation indicates that the `provider` field can create a new instance of an  `AxonTestFixture` for every test run.
...
IMPORTANT: `@ProvidedAxonTestFixture` cannot be applied on static fields, as this breaks the lifecycle contract of getting a new instance of the fixture on every test run. You can however use `@ProvideAxonTestFixture` on static and non-static methods if you want.


Alternatively, you can also parameterize `@ProvideAxonTestFixture` with a class value implementing the `@AxonTestFixtureProvider` interface. By doing so, you can reuse the same provider across multiple test classes, or even share it across your entire test suite by using a common base class.
By using the `@ProvideAxonTestFixture` directly on a test method, you can override the default configured on class level.

[source,java]
----
@ExtendWith(AxonTestFixtureExtension.class)
@ProvideAxonTestFixture(GlobalFixtureProvider.class) <1>
class AccountEntityTest {
    ...
    @Test
    @ProvideAxonTestFixture(SpecificFixtureProvider.class) <3>
...
class GlobalFixtureProvider implements AxonTestFixtureProvider {
    @Override
    public AxonTestFixture createFixture() {
...
<1> The `@ProvideAxonTestFixture(MyFixtureProvider.class)` annotation tells the extension to use the `MyFixtureProvider` class to create new instances of the `AxonTestFixture` for every test run. This allows you to centralize your fixture configuration in one place and reuse it across multiple test classes.
