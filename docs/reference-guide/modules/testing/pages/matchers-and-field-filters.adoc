...
1. Then-satisfies - Lambda-based verification providing the message(s) to assert, like xref:basic-testing.adoc#then_event_satisfies[`eventsSatisfy(...)`] and xref:basic-testing.adoc#then_command_satisfies[`commandsSatisfy(...)`].
Captures thrown `AssertionErrors` by the lambda to conclude the verification outcome.
2. Then-matches - Predicate-based verification providing the message(s) to assert, like xref:basic-testing.adoc#then_event_matches[`eventsMatch(...)`] and xref:basic-testing.adoc#then_command_matches[`commandsMatch(...)`].
