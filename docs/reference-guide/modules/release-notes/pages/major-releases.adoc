...
* Aggregate Polymorphism has been introduced, allowing for an aggregate hierarchy to come naturally from a domain model.
 ...
 For more details on this feature, read up on it xref:commands:modeling/aggregate-polymorphism.adoc[here].
...
* We have introduced the `@CreationPolicy` annotation which you can add to `@CommandHandler` annotated methods in your aggregate.
 ...
 For further explanation read the xref:commands:command-handlers.adoc#_aggregate_command_handler_creation_policy[Aggregate Command Handler Creation Policy] section.
