...
* Issue https://github.com/AxonFramework/AxonFramework/issues/1853[1853] describes an issue where the xref:commands:command-handlers.adoc#_aggregate_command_handler_creation_policy[creation policy] `always`.
...
* Some internals have changed concerning command handling exceptions.
 ...
 This piece of information allows the xref:commands:infrastructure.adoc#_configuring_the_commandgateway[`RetryScheduler`] to retry a non-transient exception since those are retryable.
...
* By enabling the xref:commands:command-handlers.adoc#_aggregate_command_handler_creation_policy[`CreationPolicy`] for the xref:commands:infrastructure.adoc#DisruptorCommandBus[`DisruptorCommandBus`], a timing issue was introduced with handling events.
...
* Through the new xref:commands:command-handlers.adoc#_aggregate_command_handler_creation_policy[Create-or-Update]
