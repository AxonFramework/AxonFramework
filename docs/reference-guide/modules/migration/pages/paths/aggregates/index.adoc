...
How to register entities and declare modules using the new xref:understanding-architecture-principles.adoc#_the_improved_configuration_architecture_makes_configuring_your_project_easier[configurer api].
...
<4> Command Handlers for existing instances now accept an `EventAppender` parameter to publish events.
<5> `@EventSourcingHandler` remains the way to update the entity state from events, keeping the logic encapsulated.
<6> State updates are performed by applying events, ensuring the entity can be reconstructed from its history.
...
There are three common patterns for using `@EntityCreator` you need to understand and apply to your setup:
...
+
For commands that should never create a new instance, use a static handler that fails and doesn't append any events.
+
[source,java]
----
@CommandHandler
public static void handle(IssueGiftCard cmd) {
    throw new IllegalStateException("Do not create!");
}
----
