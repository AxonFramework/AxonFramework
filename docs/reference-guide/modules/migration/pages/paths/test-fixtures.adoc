...
[NOTE]
====
For sagas kept running through the `axon-legacy` module, `SagaTestFixture` (`org.axonframework.test.saga.SagaTestFixture`)
is available unchanged in `axon-legacy-test`; see xref:migration:paths/sagas.adoc[Saga migration].

Simple sagas can also be written and tested without the legacy module, as an automation, going through the
`AxonTestFixture`. Check out our xref:axon-framework-5-getting-started::implement-stateful-event-handler.adoc[getting started]
...
[source,java]
----
include::example$migration/paths/testfixtures/setup/GiftCardTest.java[tag=fixture-setup,indent=0]
----
====
...
In the example above, the static `AxonConfig.appConfigurer()` method suggests your `ApplicationConfigurer` is statically constructed for your production code. However, you can also maintain a test-specific `ApplicationConfigurer` if you like. The responsibility on this lies with the user and whichever preference they have.

In contrast to using the `ApplicationConfigurer`, Axon Framework 4 enforced duplication of the registration methods on the `AggregateTestFixture` to __approach__ what the actual application did.
...
[NOTE]
====
By default, the `AxonTestFixture` disables heavy infrastructure components, like the Axon Server connector, even if they are on the classpath.
This keeps unit tests fast and self-contained, without requiring any action on your part.

If you want your test to exercise the actual infrastructure (for example, to verify your application also behaves correctly when connected to Axon Server), use the customization parameter provided on the `AxonTestFixture#with` method to opt in with `asIntegrationTest()`:

[source,java]
----
include::example$migration/paths/testfixtures/asintegrationtest/FixtureIntegrationTestCustomization.java[tag=fixture-as-integration-test,indent=0]
----
...
[source,java]
----
include::example$migration/paths/testfixtures/springboot/GiftCardTest.java[tag=fixture-setup-spring,indent=0]
----
...
[source,java]
----
include::example$migration/paths/testfixtures/given/GiftCardTest.java[tag=given-phase,indent=0]
----
====
...
The when phase specifies the action that should trigger the behaviour you want to validate.
The example below shows what this looked like in Axon Framework 4 and how it maps to Axon Framework 5:
...
[source,java]
----
include::example$migration/paths/testfixtures/when/GiftCardTest.java[tag=when-phase,indent=0]
----
====
...
[source,java]
----
include::example$migration/paths/testfixtures/then/GiftCardTest.java[tag=then-phase,indent=0]
----
====
...
[source,java]
----
include::example$migration/paths/testfixtures/chaining/GiftCardTest.java[tag=chaining,indent=0]
----
====
...
[source,java]
----
include::example$migration/paths/testfixtures/matchers/GiftCardTest.java[tag=matcher-based-validation,indent=0]
----
====
