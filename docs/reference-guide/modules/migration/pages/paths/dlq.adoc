...
For a complete understanding of how the DLQ works in Axon Framework 5, see xref:axon-framework-reference:events:event-processors/dead-letter-queue.adoc[Dead-Letter Queue].
...
However, the embedded event entry columns have changed, making the schemas **not backward compatible**.

The following columns in the embedded event entry were renamed or removed:
...
|`payload_type`
|(removed)
|Removed—no longer stored separately
...
If you have existing dead letters in an Axon Framework 4 database that you want to preserve, you must migrate those rows before switching to Axon Framework 5.

If your dead-letter queue is empty or you are comfortable discarding existing dead letters, drop the `dead_letter_entry` table and let Axon Framework 5 recreate it.
====

The outer `DeadLetterEntry` table structure (columns `dead_letter_id`, `processing_group`, `sequence_identifier`, `sequence_index`, `enqueued_at`, `last_touched`, `processing_started`, `cause_type`, `cause_message`, `diagnostics`) is unchanged.
...
In Axon Framework 5, each Event Handling Component within a processor xref:events:event-processors/dead-letter-queue.adoc#_queue_scoping_and_naming[has its own dedicated dead-letter queue].
...
[NOTE]
====
Axon Framework 5 now also allows to xref:events:event-processors/dead-letter-queue.adoc#_applying_defaults_across_processors[apply DLQ defaults across processors] that can be overridden by individual processors.
====
...
Axon Framework 5 now supports sequence identifier caching to speed up event processing when using dead letter queues. For details on this feature see the xref:events:event-processors/dead-letter-queue.adoc#_sequence_identifier_caching[reference documentation].
