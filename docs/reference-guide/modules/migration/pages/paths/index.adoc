...
Axon Framework 5.0 introduces several architectural shifts that may require different migration strategies depending on your application's complexity and the features you use.
...
* _Message changes [@EventTag,@Event/@Command/@Query] (Coming soon)_
* _Aggregate to Entity (Coming soon)_
...
* _Message Interceptors in AF5 (Coming soon)_
...
[cols="1,1,1"]
|===
| Description | Axon 4 package name | Axon 5 package name
...
| Aggregate Identifier annotation (Legacy)
| `org.axonframework.modelling.command.AggregateIdentifier`
| `org.axonframework.modelling.command.AggregateIdentifier` (Legacy Stash)

| Saga Event Handler annotation (Legacy)
| `org.axonframework.modelling.saga.SagaEventHandler`
| `org.axonframework.modelling.saga.SagaEventHandler` (Legacy Stash)
|===
...
| Inject Entity annotation
| N/A
| `org.axonframework.modelling.annotation.InjectEntity`

| Entity Id Resolver (formerly CommandTargetResolver)
...
[cols="1,1,1"]
|===
| Description | Old Module (AF 4.x) | New Module (AF 5.x)
...
As Axon Framework 5.0 and 5.1 are the first steps in this new major version, not all features from the 4.x lineage have been fully migrated or redesigned yet.
...
- **Event Upcasters**: Event upcasting is not yet available in Axon Framework 5.0/5.1.
- **Snapshots**: The snapshotting feature for entities is not yet available in 5.0.
- **Dead-Letter Queue (DLQ)**: Support for DLQs in event processors is currently missing.
- **Event Replaying**: The ability to replay events for streaming event processors is not yet implemented.
- **Multiple Event Sources**: `MultiStreamableEventSource` support is currently not available.
