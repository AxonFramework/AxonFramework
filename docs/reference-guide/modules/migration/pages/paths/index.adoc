...
* xref:migration:paths/sagas.adoc[Saga migration]
...
* xref:migration:paths/timeouts.adoc[Timeout migration]
...
| Tracing Service Provider Interface (SPI) and decorators
...
`io.axoniq.framework.tracing.micrometer` (the Micrometer Tracing binding provided by Axoniq Framework)
|===
====

For an exhaustive list of all class-level renames and moves, please refer to the https://github.com/AxonFramework/AxonFramework/blob/main/axon-5/api-changes/11-class-reference.md[`11-class-reference.md`] file under `axon-5/api-changes/` in the Axon Framework repository.
...
| The tracing SPI and component decorators are part of the core open-source modules. Exporting spans to OpenTelemetry is provided by Axoniq Framework through its Micrometer Tracing binding (`axoniq-tracing-micrometer`).
| `axon-tracing-opentelemetry`
| Not part of the OSS Axon Framework module set.
|===
...
- **Running sagas and scheduled deadlines**: There is no `Saga` construct, `SagaStore` or `DeadlineManager` in Axon Framework 5, and no tooling to carry the contents of an Axon Framework 4 `SagaStore` or scheduled deadlines across.
  An `axon-legacy` module, scheduled for 5.4.0, keeps existing sagas and their deadlines running on Axon Framework 5 almost unchanged, so long-running processes can drain after the upgrade instead of before it.
  New processes are fully supported today: see xref:{page-component-version}@saga-guide:ROOT:index.adoc[Sagas and Process Managers] for the available approaches, and xref:migration:paths/sagas.adoc[Saga migration] for both the legacy module and the drainage strategy.
  A first-class replacement, Workflows, is in preview in Axoniq Framework.
- **Aggregate Caching**: The ability to cache aggregates hasn't returned yet. We plan to revamp this support entirely for a broader caching solution.
