...
This path covers the migration from Axon Framework 4.x annotations to the new message-centric approach in AF5.
Axon Framework 5 introduces a more explicit way of defining and handling messages.
...
This ensure compatibility with an aggregate-based storage solution, wherein events are stored with an aggregate `type` column.
