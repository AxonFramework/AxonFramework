...
All to accommodate the xref:events:event-store-internals.adoc#dynamic_consistency_boundaries_dcb[Dynamic Consistency Boundary] **and** the previous aggregate-based solution.
...
In Axon Framework 4, this role was taking by the `AxonServerEventStore`.
...
Axon Framework's JPA event storage solution remains on the aggregate-based approach instead of supporting DCB.
...
2. **Rename existing table and columns**: Alter your existing `domain_event_entry` table to match the new schema.
...
-- Instead, rename the existing sequence and adjust its settings.
...
-- If renaming is not possible, create a new sequence starting from the current max globalIndex + 1:
-- CREATE SEQUENCE aggregate_event_global_index_sequence
--     START WITH (SELECT COALESCE(MAX(globalIndex), 0) + 1 FROM aggregate_event_entry)
--     INCREMENT BY 1;

-- MySQL example (uses AUTO_INCREMENT, no sequence rename needed, but verify the column is configured correctly):
...
The, with the `EventSourcingConfigurer`, we can register the `AggregateBasedJpaEventStorageEngine`:
...
[NOTE]
====
The `PostgresqlEventStorageEngine` is scheduled for release in 2026 Q1.
