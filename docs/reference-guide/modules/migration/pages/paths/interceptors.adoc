...
* **Clear separation**: Distinct interfaces for dispatch vs. handler interception
...
+
* `runOnStart(Consumer<ProcessingContext>)` - Execute when context starts
* `runOnAfterCommit(Consumer<ProcessingContext>)` - Execute after successful commit
* `runOnRollback(Consumer<ProcessingContext>)` - Execute on rollback
* `onError(ProcessingErrorHandler)` - Handle errors during processing
====

== MessageDispatchInterceptor interface changes

Dispatch interceptors intercept messages **before** they reach the handler, even before a `ProcessingContext` is created.
...
3. Registers them with the appropriate registries based on their generic type
...
* `Class<?> componentType` - The component class (for example, aggregate, projection)
