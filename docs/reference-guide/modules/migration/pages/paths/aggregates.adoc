...
* **`@EntityCreator`**: A mandatory annotation that tells Axon how to instantiate your entity. In AF4, the framework often relied on a `@CommandHandler` constructor; in AF5, the instantiation is decoupled from the command handling logic.
...
In AF4, the `@AggregateIdentifier` annotation was used on a field in the aggregate to mark it as the identifier. 
In AF5, the identifier is typically specified during the configuration of the entity, or injected into the `@EntityCreator`.
...
=== Import Changes for Annotations
...
AF4 often used constructors annotated with `@CommandHandler` to create new aggregate instances. 
AF5 favors **static factory methods** for command handling of creation commands, combined with the `@EntityCreator` annotation for instance instantiation. This is the recommended way to migrate your creation constructors.
...
The `@EntityCreator` annotation is a **hard requirement** in AF5 for event-sourced entities. It tells Axon how to create an initial instance of the entity before applying events to it.
...
public GiftCard() { }
----
Axon creates an empty instance. The identifier and other state are then initialized in the `@EventSourcingHandler` for the first event.
...
The `@AggregateRoot` annotation is replaced by `@EventSourcedEntity`. In AF5, this annotation (or `@EventSourced` in Spring) is a requirement for the framework to recognize and manage the entity.
...
In AF4, polymorphic aggregates were configured using `@Aggregate` on the base class and sub-classes. In AF5, you register the hierarchy through the `EventSourcedEntityModule`.

[source,java]
----
// AF5 Configuration snippet
...
The declarative configuration of entities now uses the `EventSourcedEntityModule` instead of `AggregateConfigurer`. This aligns with the new modular configuration architecture of AF5.
