...
<1> When `namespace` is not specified on `@Event`, Axon first searches for the presence of the `@Namespace` annotation (as described xref:messaging-concepts:anatomy-message.adoc#namespace-annotation[her]). When no `@Namespace` is found, the `namespace` falls back to the package name of the class.

The event handler's name is composed of `namespace` + `name` (for example, `"orders.OrderPlaced"`).
