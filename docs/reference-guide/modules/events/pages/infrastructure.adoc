...
For more details on event stores specifically, see the xref:events:event-store-specifics.adoc[event store specifics] page.
...
For more details on how to publish events with the event gateway, read the following xref:event-dispatchers.adoc#dispatching_events_from_a_non_entity[section].
...
For more details on how to publish events with an `EventAppender`, read the following xref:event-dispatchers.adoc#dispatching_events_from_an_entity[section].
...
For detailed information on event store configuration, Dynamic Consistency Boundaries (DCB), event tags, and consistency markers, see the xref:event-store-specifics.adoc[Event Store Specifics] page.
