...
As correctly sequencing your events is important for any application, we **highly** you check out the <<sequential_processing,sequential processing>> section to understand how to configure a policy that fits with your application.
...
Consider the following snippets when configuring a (custom) `SequencingPolicy` and for using the `@SequencingPolicy` annotation:
...
class MyEventHandlingComponent {
...
By far the simplest approach is to connect your application(s) to Axoniq Platform, as it provides the most flexibility.
