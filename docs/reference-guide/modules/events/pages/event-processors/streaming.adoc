...
The `assigningHandlers` method uses the `EventHandlerSelector` to receives an `EventHandlerDescriptor` that provides access to:
...
In the example above, we use the convenience `EventHandlerSelector` that assigns handlers that are annotated with `@Namespace("orders")`.
Handlers can be assigned with a `@Namespace` on several levels, as described xref:ROOT:spring-boot-integration.adoc#namespace-based-handler-selection[here].
When the Event Processor name and `@Namespace` name are identical, you can use the `EventProcessorDefinition.pooledStreamingMatching()` to skip the `assigningHandlers` method automatically:
