...
The provided function receives a `TrackingTokenSource` which provides methods to create tokens at different positions:
...
The `claimExtensionThreshold` determines how long to wait before extending a claim in the absence of events, while `tokenClaimInterval` defines how frequently the processor attempts to claim segments.
...
When using `ProcessorDefinition` with autodetected event handlers, you can define the processor configuration:
...
public class EventProcessorConfig {
    ...
    public ProcessorDefinition exampleProcessorDefinition() {
        return ProcessorDefinition.pooledStreamingProcessor("example-processor")
                .assigningHandlers(descriptor ->
                    descriptor.beanName().equals("customEventHandlingComponent"))
                .withDefaultSettings();
    }
}
----
When using `ProcessorDefinition` with Spring Boot, sequencing policies should still be configured using the `@SequencingPolicy` annotation on the event handler class or methods (as shown <<Sequencing Policies,earlier in this chapter>>).
The programmatic configuration through `ProcessorDefinition` focuses on processor-level settings, while sequencing policies are defined at the handler level.
...
The `coordinatorExecutor` manages segment claiming and coordination (typically 1 thread suffices), while the `workerExecutor` handles actual event processing.
Increase the worker thread pool size to match your desired parallelism level.
--
...
[#common_configuration_scenarios]
== Common configuration scenarios with ProcessorDefinition

This section provides complete examples for common event processor configuration scenarios using `ProcessorDefinition` in Spring Boot applications.

[#scenario_high_throughput]
=== High-throughput order processing

Configure a processor optimized for high-throughput order event processing with increased parallelism and batching:
...
public class OrderEventProcessorConfig {
    ...
    public ProcessorDefinition orderProcessor() {
        return ProcessorDefinition.pooledStreamingProcessor("order-processor")
                ...
                .withConfiguration(config -> config
                    .initialSegmentCount(32)  // High parallelism
                    .batchSize(100)            // Process events in batches
                    .tokenClaimInterval(3000)
                    .claimExtensionThreshold(3000));
    }
}
----

[#scenario_multiple_processors]
=== Multiple domain-specific processors

Separate processors for different bounded contexts, each with tailored configuration:
...
@Configuration
public class DomainEventProcessorConfig {
    ...
    @Bean
    public ProcessorDefinition inventoryProcessor() {
        return ProcessorDefinition.pooledStreamingProcessor("inventory-processor")
                .assigningHandlers(descriptor ->
                    descriptor.beanType().getPackageName()
                              .startsWith("com.example.inventory"))
                .withConfiguration(config -> config
    ...
    @Bean
    public ProcessorDefinition shippingProcessor() {
        return ProcessorDefinition.pooledStreamingProcessor("shipping-processor")
                .assigningHandlers(descriptor ->
                    descriptor.beanType().getPackageName()
                              .startsWith("com.example.shipping"))
                .withConfiguration(config -> config
                    .initialSegmentCount(4)
                    .batchSize(10));
    }
...
[#scenario_custom_token_store]
=== Custom token store per processor

Use different token stores for different processors to separate concerns or optimize storage:
...
public class TokenStoreConfig {
    ...
    @Bean
    public TokenStore orderTokenStore(EntityManagerProvider entityManagerProvider,
                                      Converter converter) {
    ...
    @Bean
    public ProcessorDefinition orderProcessor(TokenStore orderTokenStore) {
        return ProcessorDefinition.pooledStreamingProcessor("order-processor")
                .assigningHandlers(descriptor ->
                    descriptor.beanName().startsWith("order"))
                .withConfiguration(config -> config
...
[#scenario_latest_events]
=== Starting from latest events

Configure a processor to start from the latest position, skipping historical events:
...
@Configuration
public class RealtimeEventProcessorConfig {

    @Bean
    public ProcessorDefinition realtimeProcessor() {
        return ProcessorDefinition.pooledStreamingProcessor("realtime-processor")
                .assigningHandlers(descriptor ->
                    descriptor.beanName().contains("realtime"))
                .withConfiguration(config -> config
                    .initialToken(source -> source.latestToken())
                    .initialSegmentCount(8));
    }
}
----
...
[NOTE]
====
The `initialToken` configuration only applies when the processor starts for the first time.
If tracking tokens already exist, the processor will resume from its last known position.
====
...
====
The `MultiStreamableEventSource` feature is not yet available in Axon Framework 5.0.
It will be reintroduced in Axon Framework 5.2.0.
The documentation below describes the multiple event sources concepts for reference and future use.
====
...
----
import org.axonframework.extension.spring.config.ProcessorDefinition;
import org.axonframework.messaging.eventhandling.MultiStreamableEventSource;
import org.axonframework.messaging.eventhandling.TrackedEventMessage;
import org.axonframework.messaging.eventhandling.tokenstore.StreamableEventSource;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
