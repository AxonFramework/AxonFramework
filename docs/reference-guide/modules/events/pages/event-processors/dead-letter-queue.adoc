...
public class AxonConfig {
    ...
    public void configureDeadLetterQueue(EventProcessingConfigurer processingConfigurer) {
        // Replace "my-processor" for the processor you want to configure the DLQ on.
...
public class AxonConfig {
    // omitting other configuration methods...
    @Bean
    public ConfigurationEnhancer deadLetterQueueConfigurationEnhancer() {
        // Replace "my-processor" for the processor you want to configure the DLQ on.
        return registry -> MessagingConfigurer.with(registry).eventProcessing().registerDeadLetterQueue(
                "my-processor",
                config -> JpaSequencedDeadLetterQueue.builder()
                                                     .processingGroup("my-processor")
                                                     .maxSequences(256)
...
To make it easier to use a dead-letter queue on multiple processors, it's possible to set a dead-letter queue provider.
The provider is a function that takes a processor name, and returns either `null`, meaning it will not be configured using a dead-letter queue, or a function that takes the `Configuration` and returns a new dead-letter queue.

Here is a `JpaSequencedDeadLetterQueue` configuration example that uses a collection to determine if a dead-letter queue should be created for a given processor:
...
public class AxonConfig {
    ...
    public void configureDeadLetterQueue(EventProcessingConfigurer processingConfigurer) {
        processingConfigurer.registerDeadLetterQueueProvider(
                processorName -> {
                    //dlqEnabledProcessors is a collection with the processors that should have a dlq
                    if (dlqEnabledProcessors.contains(processorName)) {
...
public class AxonConfig {
    ...
    @Bean
    public ConfigurationEnhancer deadLetterQueueConfigurationEnhancer () {
        return registry -> MessagingConfigurer.with(registry).eventProcessing().registerDeadLetterQueueProvider(
                processorName -> {
                    //dlqEnabledProcessors is a collection with the processors that should have a dlq
                    if (dlqEnabledProcessors.contains(processorName)) {
                        return config -> JpaSequencedDeadLetterQueue.builder()
...
For example, by setting the `axon.eventhandling.processors.my-processor.dlq.enabled` to true you would enable the dead-letter queue for the `my-processor` processor.
...
You can retrieve a `SequencedDeadLetterProcessor` from the `EventProcessingConfiguration` based on a processor name _if_ you have configured a dead-letter queue for this processor.
    ...
    public void retryAllSequences(String processingGroup) {
       ...
       Iterable<Iterable<DeadLetter<? extends EventMessage<?>>>> deadLetterSequences =
               config.deadLetterQueue(processingGroup)
                     ...
                     .orElseThrow(() -> new IllegalArgumentException("No such processor"));
...
public class AxonConfig {
    ...
    public void configureEnqueuePolicy(EventProcessingConfigurer configurer) {
        // Replace "my-processor" for the processor you want to configure the policy on.
...
public class AxonConfig {
    // omitting other configuration methods...
    @Bean
    public ConfigurationEnhancer enqueuePolicyConfigurationEnhancer() {
        // Replace "my-processor" for the processor you want to configure the policy on.
