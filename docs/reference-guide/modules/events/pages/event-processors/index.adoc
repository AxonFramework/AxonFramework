...
Event handlers are assigned directly to an Event Processor by name.
Each event handler, or "Event Handling Component," belongs to a single Event Processor.
The Event Processor manages all aspects of event processing including threading, <<listener-invocation-error-handler,error handling>>, xref:event-processors/streaming.adoc#sequential-processing[sequencing], and transaction management.
...
Using the package name serves as a suitable default, but using dedicated names for Event Processors is recommended.
The most straightforward approach to reaching a transparent naming scheme is by using the `ProcessingGroup` annotation.
This annotation assigns a handler to a specific processor by name.

The `ProcessingGroup` annotation requires the insertion of a name and can only be set on the class.
Let us adjust the previous sample by using this annotation to explicitly assign handlers to processors:
...
The Configuration API allows you to configure strategies for assigning event handling classes to processors or assigning specific handler instances to particular processors.
Below is a list of the assignment rules available for configuring event handler to processor assignment:

*Default assignment strategies*

* `byDefaultAssignTo(String)`: defines the default processor name to assign an event handler to.
It will only be taken into account if there are no more specific rules and if the `ProcessingGroup` annotation is not present.
* `byDefaultAssignHandlerInstancesTo(Function<Object, String>)`: defines a lambda invoked to assign an event handling instance to a desired processor by returning that processor's name.
It will only be taken into account if there are no more specific rules and if the `ProcessingGroup` annotation is not present.
* `byDefaultAssignHandlerTypesTo(Function<Class<?>, String>)`: defines a lambda invoked to assign an event handler type to a desired processor by returning that processor's name.
It will only be taken into account if there are no more specific rules and if the `ProcessingGroup` annotation is not present.

*Predicate-based assignment*

* `assignHandlerInstancesMatching(String, Predicate<Object>)`: assigns event handlers to the given processor name based on a predicate ingesting an event handling instance.
The operation uses a natural priority of zero.
If an instance matches several criteria, the outcome is _undefined_.
* `assignHandlerTypesMatching(String, Predicate<Class<?>>)`: assigns event handlers to the given processor name based on a predicate ingesting an event handler type.
The operation uses a natural priority of zero.
If an instance matches several criteria, the outcome is _undefined_.
* `assignHandlerInstancesMatching(String, int, Predicate<Object>)`: assigns event handlers to the given processor name based on a predicate ingesting an event handling instance.
Uses the given priority to decide on rule-ordering.
The higher the priority value, the more important the rule is.
If an instance matches several criteria, the outcome is _undefined_.
* `assignHandlerTypesMatching(String, int, Predicate<Class<?>>)`: assigns event handlers to the given processor name based on a predicate ingesting an event handler type.
Uses the given priority to decide on rule-ordering.
...
By default, exceptions raised by event handlers are caught by the <<listener-invocation-error-handler,listener invocation error handler>>, logged, and processing continues with the following events.
...
To change this behavior, Event Processors allow customization on how to deal with exceptions:
...
The default `ListenerInvocationErrorHandler` can be customized globally or per event processor:
...
public class AxonConfig {
    ...
    public void configureErrorHandling(EventProcessingConfigurer processingConfigurer) {
        ...
        processingConfigurer
            ...
            .registerListenerInvocationErrorHandler("my-processor", conf -> {
                 /* create listener error handler */
            });
    }
}
...
public class AxonConfig {
    // omitting other configuration methods...
    @Bean
    public ConfigurationEnhancer errorHandlingConfigurationEnhancer() {
...
The exception will bubble up to the <<event-processor-error-handler,processor's error handler>> when rethrown.
...
public class AxonConfig {
    @Bean
    public ConfigurationEnhancer processorErrorHandlingConfigurationEnhancer() {
        return registry -> MessagingConfigurer.with(registry).eventProcessing(processing ->
            processing
...
//=== Dead-letter queue
//
// TODO: Reinstate when DLQ API is available in Axon 5.0
...
Axon uses the `TransactionManager` to attach a transaction to every xref:messaging-concepts:processing-context.adoc[Unit of Work].
...
It is the `RollbackConfiguration` that decide when a xref:messaging-concepts:processing-context.adoc[Unit of Work] should rollback the transaction.
The default `RollbackConfiguration` is to rollback on any type of `Throwable`; the xref:messaging-concepts:processing-context.adoc[Unit of Work] page describes the other options you can choose.
