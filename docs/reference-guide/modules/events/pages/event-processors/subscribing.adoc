...
2. <<bean_based_config_spring,Bean-based configuration>> - Using `ProcessorDefinition` beans in Spring Boot
...
When using Spring Boot, you can define `ProcessorDefinition` beans to configure subscribing event processors.
...
public class EventProcessorConfig {
    ...
    @Bean
    public ProcessorDefinition exampleProcessorDefinition(EventBus eventBus) {
        return ProcessorDefinition.subscribingProcessor("example-processor")
...
public class EventProcessorConfig {
    ...
    @Bean
    public ProcessorDefinition advancedProcessorDefinition(
            EventBus eventBus,
            MessageHandlerInterceptor<? super EventMessage> interceptor) {
        return ProcessorDefinition.subscribingProcessor("advanced-processor")
                .assigningHandlers(descriptor ->
...
1. `ProcessorDefinition` beans take precedence over properties-based configuration for assigned handlers
2. If a handler matches multiple `ProcessorDefinition` selectors, an `AxonConfigurationException` is thrown
3. Handlers not matched by any `ProcessorDefinition` are assigned to a processor named after their package
...
For a specific Event Processor to be a Subscribing instance using a persistent stream, you can use the Configuration API, Spring Boot with `ProcessorDefinition`, or Spring Boot properties files.
...
When using Spring Boot, you can define `ProcessorDefinition` beans along with a `PersistentStreamMessageSource` bean:
...
public class EventProcessorConfig {
    ...
    @Bean
    public ProcessorDefinition persistentStreamProcessor(
            PersistentStreamMessageSource persistentStreamSource) {
        return ProcessorDefinition.subscribingProcessor("persistent-stream-processor")
                .assigningHandlers(descriptor ->
                    descriptor.beanType().getPackageName()
