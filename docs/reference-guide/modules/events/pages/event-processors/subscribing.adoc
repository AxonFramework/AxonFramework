...
public class AxonConfig {
    // omitting other configuration methods...
    @Bean
    public ConfigurationEnhancer processorDefaultConfigurerModule() {
        return registry -> MessagingConfigurer.with(registry).eventProcessing(EventProcessingConfigurer::usingSubscribingEventProcessors);
    }
}
----
...
public class AxonConfig {
    // omitting other configuration methods...
    @Bean
    public ConfigurationEnhancer subscribingProcessorsConfigurerModule() {
...
To simplify configuration, if you want to apply persistent streams to all processors all you have to do is set property:
...
This will configure persistent streams automatically for all processors with default settings.
