...
The xref:event-handlers.adoc[event handlers] pages provide the background on how to handle event messages in your application.
The publishing process is the starting point of such an event message.
...
. Publishing events from an entity, and
...
1. The `EventAppender` - This component comes into play when <<event_appender,publishing events from an entity>>.
