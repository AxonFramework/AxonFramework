...
The xref:commands:modeling/aggregate.adoc[Aggregate] or its xref:commands:modeling/multi-entity-aggregates.adoc[Entities] are typically the starting point of all event messages.
...
In the vast majority of cases, the xref:commands:modeling/aggregate.adoc[Aggregates] will publish events by applying them.
