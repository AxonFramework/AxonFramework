...
With these modules you can register a number of `MessageMonitor` implementations to your messaging components, like the xref:commands:command-dispatchers.adoc#the-command-bus[`CommandBus`], xref:events:event-bus-and-event-store.adoc#_event_bus[`EventBus`], xref:queries:query-dispatchers.adoc#_the_query_bus_and_query_gateway[`QueryBus`] and xref:events:event-processors/index.adoc[`EventProcessors`].
