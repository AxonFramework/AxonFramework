...
:navtitle: Event Processor Monitoring
...
[TIP]
.Out-of-the-box status monitoring
====
AxonIQ Platform and Axon Server's dashboard use this exact tracker status to show users how event processors are behaving. Although you can implement custom tooling using the `StreamingEventProcessor#processingStatus` method, you can save time by using either of these tools.
====
...
* A boolean through `isMerging()` specifying whether the given Segment is xref:events:event-processors/streaming.adoc#splitting_and_merging_segments[merging].
...
The exposed metrics can be scraped by the tool of your choice (for example, link:https://prometheus.io/[Prometheus]) and alerting can be put in place for several useful metrics.
