...
The `CommandGateway` is a convenience approach towards dispatching commands.
...
Dispatching commands generally has two possible outcomes:

. The command is handled successfully.
. The command is handled exceptionally.

The outcome depends on the implementation of the command handler.
If the `@CommandHandler` annotated method throws an exception due to some business logic, it will be that exception which will be the result of dispatching the command.
...
The framework has built-in support for this: when a `@CommandHandler` annotated static handler executes successfully, the value of the `@EntityId` annotated field is automatically returned.
