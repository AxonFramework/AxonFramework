...
====
If your application uses Project Reactor (for example, Spring WebFlux), the xref:axon-framework-reference:ROOT:modules.adoc#axon-reactor[Reactor extension] provides a `ReactiveCommandGateway` that returns `Mono<R>` instead of `CompletableFuture`.
