...
public class GiftCard {
    ...
    public void handle(RedeemCardCommand cmd, EventAppender eventAppender) {
        ...
        if (cmd.amount() > remainingValue) { // <6>
...
<3> The `@EntityId` annotation marks the field containing the unique identifier used to load the correct instance.

<4> A creational command handler (static method) creates a new instance by appending events.

<5> An instance command handler (instance method) operates on an existing instance, requiring it to be loaded first.

<6> Instance handlers have access to current state for decision-making.
...
public void handle(
    ...
    String traceId = context.correlationData().traceId();
...
----
    void buildConfiguration() {
        ApplicationConfigurer configurer = ApplicationConfigurer.defaultConfiguration();

        // Register stateful handler
        configurer.modellingConfigurer(modellingConfigurer ->
            modellingConfigurer.eventSourcingConfigurer(eventSourcingConfigurer ->
                eventSourcingConfigurer.registerEntity(GiftCard.class)
            )
        );

        // Register stateless handler
        configurer.messagingConfigurer(messagingConfigurer ->
            messagingConfigurer.registerCommandHandler(config ->
                new GiftCardValidationHandler(config.queryDispatcher())
            )
        );
    }
----
