...
<1> The `namespace` typically represents the bounded context. When `namespace` is not specified on `@Command`, Axon searches for the presence of the `@Namespace` annotation (as described xref:messaging-concepts:anatomy-message.adoc#namespace-annotation[her]). When no `@Namespace` is found, the `namespace` falls back to the package name of the class.
