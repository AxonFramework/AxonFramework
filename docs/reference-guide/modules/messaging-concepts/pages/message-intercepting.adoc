...
When using the xref:axon-framework-reference:ROOT:modules.adoc#axon-reactor[Reactor extension], reactor gateways provide their own interceptor type: `ReactorMessageDispatchInterceptor`.
...
ReactorMessageDispatchInterceptor<CommandMessage> securityInterceptor() {
    return (message, context, chain) -> Mono.deferContextual(ctx -> { // <2>
...
<1> Declare the interceptor as a Spring bean.
...
<3> The interceptor enriches the message metadata and proceeds through the chain, passing along the `ProcessingContext`.
