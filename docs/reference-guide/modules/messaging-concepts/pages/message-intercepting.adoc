...
When using the xref:axon-framework-reference:ROOT:modules.adoc#axon-reactor[Reactor extension], reactive gateways provide their own interceptor type: `ReactiveMessageDispatchInterceptor`.
Unlike the standard `MessageDispatchInterceptor`, the reactive variant runs inside the Reactor subscription chain.
...
[source,java]
----
ReactiveMessageDispatchInterceptor<CommandMessage> securityInterceptor =
    (message, chain) -> Mono.deferContextual(ctx -> { // <1>
...
reactiveCommandGateway.registerDispatchInterceptor(securityInterceptor); // <3>
----

<1> `Mono.deferContextual()` accesses the Reactor context from the subscriber.
In a Spring WebFlux application, this is where `ReactiveSecurityContextHolder` stores the authenticated user.
<2> The interceptor enriches the message metadata and proceeds through the chain.
<3> Register the interceptor on the reactive gateway.
