...
Usually, correlation is implemented using specific message properties—correlation identifiers that track relationships between messages.
...
The `ProcessingContext` is in charge of populating the metadata of new messages based on configured correlation data providers.
...
It is responsible for propagating two values from one message to another: the `correlationId` and `causationId`.
...
public void handle(PlaceOrderCommand command) {
    ...
    // Publish event
    EventMessage event = // ... create event
...
public void on(OrderPlacedEvent event, ProcessingContext context) {
    ...
    // Send command
    CommandMessage command = // ... create command
...
public void buildConfiguration() {
    MessagingConfigurer.create()
        .registerCorrelationDataProvider(config -> new MessageOriginProvider())
