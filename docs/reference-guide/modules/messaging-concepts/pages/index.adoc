...
They are implemented as POJOs that are wrapped in a `CommandMessage`.  Axon Framework refers to your command as the "command payload".
Thus, to retrieve it from a `CommandMessage` you use the `CommandMessage#payload` method.
...
Auditing information is a typical example - it allows you to see under which circumstances an command was raised, such as the user account that triggered the processing, or the name of the machine that processed
the command.
