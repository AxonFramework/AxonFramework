...
====
All messages are immutable.
Storing data in a message means creating a new message based on the previous one, with extra information added to it.
...
They are implemented as POJOs that are wrapped in a `CommandMessage`.
...
Auditing information is a typical example - it allows you to see under which circumstances an event was raised, such as the user account that triggered the processing, or the name of the machine that processed the event.
...
When dispatching an event on the event bus, you need to wrap it in an event message.
The `GenericEventMessage` is an implementation that allows you to wrap your event in a message using its constructor.
Axon will automatically wrap events in `EventMessage` instances when they are published.

== Queries

Queries describe a request for information or state.
A query can have multiple handlers.
When dispatching queries, the client indicates whether it wants a result from one handler or from all available query handlers.
...
When you send a command or dispatch a query, the framework handles the complexity of async processing for you.
...
Using xref:axoniq-console-reference:ROOT:index.adoc[AxonIQ Platform], you can monitor the flow of messages in your application as you can see in the image below.
