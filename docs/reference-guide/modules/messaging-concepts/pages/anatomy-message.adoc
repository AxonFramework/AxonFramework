...
While you can specify the `namespace` attribute directly on each `@Command`, `@Event`, or `@Query` annotation, Axon provides the `@Namespace` annotation as a convenient way to define the namespace for multiple message classes at once.

The `@Namespace` annotation can be applied at different levels:

* **On a class** - Defines the namespace for that specific message class. Technically no different from
...
Thus, to automatically set the namespace for several message inside the same package, we could annotate the `package-info.java` file like so:
...
The different levels just described also represent the search order. Thus, when resolving the namespace for a class, the following order is taken until a non-empty value is found:

1. The `namespace` attribute on the message annotation (`@Command`, `@Event`, or `@Query`)
2. The `@Namespace` annotation on the class itself
...
[TIP]
====
Using `@Namespace` at the package level is particularly useful for Domain-Driven Design (DDD) applications, where you can align the namespace with your bounded context.
All messages within a bounded context can share the same namespace, making it explicit which context they belong to.
====
...
* If you don't specify a `namespace`, Axon searches for the `@Namespace` (as described xref:namespace-annotation[here]) to use its value as the `namespace`.
The fallback for when that is not present too is the package name of the class.
