...
[source,java]
----
void testWithStubProcessingContext() {
    CommandMessage command = new GenericCommandMessage(new MessageType("command"), new PlaceOrderCommand());
    // Create context with a message
    ProcessingContext contextMessageOnly = StubProcessingContext.forMessage(command);

    CommandGateway mockGateway = mock(CommandGateway.class);
    ...
    stub.onCommit(ctx -> verifyCommitPhase());
    stub.moveToPhase(ProcessingLifecycle.DefaultPhases.COMMIT).join();
}
----