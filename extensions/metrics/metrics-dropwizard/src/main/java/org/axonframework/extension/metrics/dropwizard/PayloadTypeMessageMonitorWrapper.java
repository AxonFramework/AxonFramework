...
public class PayloadTypeMessageMonitorWrapper<T extends MessageMonitor<Message> & MetricSet>
    ...
    /**
     * Create a PayloadTypeMessageMonitorWrapper which builds monitors through a given {@code monitorSupplier} for every
     * message payload type encountered.
     *
    ...
    /**
     * Create a PayloadTypeMessageMonitorWrapper which builds monitors through a given {@code monitorSupplier} for every
     * message payload type encountered and sets the monitor name as specified by the {@code monitorNameBuilder}.
     *
     * @param monitorSupplier    A Supplier of MessageMonitors of type {@code T} for every encountered payload type
     * @param monitorNameBuilder A Function where the payload type is the input (of type {@code Class<?>}) and output is
     *                           the desired name for the monitor (of type {@code String})
     */
    public PayloadTypeMessageMonitorWrapper(Supplier<T> monitorSupplier,
    ...
    public Map<MetricName, Metric> getMetrics() {
        ...
        payloadTypeMonitors.forEach((name, monitor) -> result.put(MetricName.build(name), monitor));
