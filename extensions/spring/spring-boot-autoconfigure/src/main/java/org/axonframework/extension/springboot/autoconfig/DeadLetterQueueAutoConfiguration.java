...
/**
 * Spring Boot autoconfiguration that provides a {@link PooledStreamingEventProcessorModule.Customization} for Dead
 * Letter Queue support.
 * <p>
 * This configuration is implementation-agnostic — it reads per-processor DLQ settings from
 * {@link DeadLetterQueueProcessorProperties} and configures the {@link DeadLetterQueueConfiguration} extension. The
 * actual queue backend (JPA, JDBC, etc.) is provided by a separate autoconfiguration that registers a
 * {@link SequencedDeadLetterQueueFactory} bean.
 *
...
public class DeadLetterQueueAutoConfiguration {
    ...
    /**
     * Creates a {@link PooledStreamingEventProcessorModule.Customization} that enables the Dead Letter Queue extension
