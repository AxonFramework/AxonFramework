...
public class AggregateBasedConsistencyMarker extends AbstractConsistencyMarker<AggregateBasedConsistencyMarker> {
    ...
    /**
     * This constains the consistency marker positions per aggregate. Note that <b>positions</b> are not the
     * same as <b>sequence numbers</b>, but are closely related: positions are one higher than the corresponding
     * sequence number.
     */
    ...
    public final class AggregateSequencer {
        ...
        public long incrementAndGetSequenceOf(String aggregateIdentifier) {
            return aggregateSequences.compute(
                    aggregateIdentifier,
                    (key, oldValue) -> oldValue == null ? sequenceNumberOf(key) + 1 : oldValue + 1
            );
        }

        private long sequenceNumberOf(String aggregateIdentifier) {
            // As positions (from the marker) are one higher than sequence numbers, and we're now going
            // to track a sequence number in aggregateSequences, be sure to subtract one here:
            return aggregatePositions.getOrDefault(aggregateIdentifier, 0L) - 1;
        }
    }
}
