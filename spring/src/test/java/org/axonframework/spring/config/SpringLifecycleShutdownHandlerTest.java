...
class SpringLifecycleShutdownHandlerTest {
    ...
    void setUp() {
        ...
        doReturn(CompletableFuture.completedFuture(null)).when(action).get();
    }

    @Test
    void testPhaseIsRegisteredCorrectly() {
        assertEquals(42, testSubject.getPhase());
    }

    @Test
    void testIsRunningReflectsCorrectState() {
    ...
    @Test
    void testActionIsInvokedOnShutdown() {
        testSubject.stop();
        verify(action).get();
    }

    @Test
    void testCallbackInvokedOnAsyncShutdown() {
