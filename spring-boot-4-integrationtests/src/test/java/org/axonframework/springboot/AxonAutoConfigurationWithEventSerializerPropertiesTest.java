...
import org.axonframework.serialization.jackson3.JacksonSerializer3;
import org.axonframework.serialization.jackson3.JacksonSerializer3Customizer;
...
class AxonAutoConfigurationWithEventSerializerPropertiesTest {
    ...
    @Test
    void eventSerializerIsOfTypeJacksonSerializer3AndUsesDefinedCustomizerBean() {
        final Serializer serializer = applicationContext.getBean(Serializer.class);
        final Serializer eventSerializer = applicationContext.getBean("eventSerializer", Serializer.class);

        assertTrue(serializer instanceof JacksonSerializer3);
        assertEquals(((JacksonSerializer3) serializer).getObjectMapper().deserializationConfig().getLocale(), Locale.CANADA_FRENCH);

        assertTrue(eventSerializer instanceof JacksonSerializer3);
        assertEquals(((JacksonSerializer3) eventSerializer).getObjectMapper().deserializationConfig().getLocale(), Locale.CANADA_FRENCH);
    }

    @Configuration
    public static class TestContext {
        @Bean
        public JacksonSerializer3Customizer customizer() {
            return builder -> builder.defaultLocale(Locale.CANADA_FRENCH);
        }
    }
}
