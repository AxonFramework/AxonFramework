...
/**
 * Class that overrides the default FireEventJob from Axon 3 to be able to restore Axon 2 Quartz triggers from Axon 3.
 */
public class FireEventJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        try {
            ...
            Object event = Optional.ofNullable(jobDataBinder.fromJobData(jobData)).orElse(jobData.get("org.axonframework.domain.EventMessage"));
    ...
    private EventMessage<?> createMessage(Object event) {
        ...
        } else if (event instanceof org.axonframework.domain.GenericEventMessage) {
            eventMessage = new GenericEventMessage<>(
                    ((org.axonframework.domain.GenericEventMessage) event).getPayload(),
                    new MetaData(((org.axonframework.domain.GenericEventMessage) event).getMetaData().getValues())
            );
        } else {
