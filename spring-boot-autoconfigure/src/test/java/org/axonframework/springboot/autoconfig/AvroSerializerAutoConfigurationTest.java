...
class AvroSerializerAutoConfigurationTest {
    ...
    void avroSerializerAutoConfigurationConstructsSchemaStore() {
        testApplicationContext.withUserConfiguration(DefaultContext.class)
            ...
            .run(context -> {
                ...
                assertThat(context).getBean("specificRecordBaseClasspathAvroSchemaLoader").isInstanceOf(ClasspathAvroSchemaLoader.class);
            });
    }
        ...
        testApplicationContext.withUserConfiguration(DefaultContext.class)
            ...
            .run(context -> {
                ...
                ComplexObject complexObject = ComplexObject.newBuilder()
                    ...
                    .setValue3(42)
                    .build();
                SerializedObject<byte[]> serialized = avroSerializer.serialize(complexObject, byte[].class);
                ...
                ComplexObject deserializedAfterUpcasting = avroSerializer.deserialize(
                  new SimpleSerializedObject<>(genericRecord, GenericRecord.class, serialized.getType())
                );
    ...
    void axonAutoConfigurationConstructsAvroSerializerThatIsNotAbleToSerializeBecauseNoSchemasAreFound() {
        testApplicationContext.withUserConfiguration(ContextWithoutSchemaScan.class)
            .withPropertyValues("axon.serializer.events=avro")
            .run(context -> {
                Serializer serializer = context.getBean("eventSerializer", Serializer.class);
                assertInstanceOf(AvroSerializer.class, serializer);
                ...
                ComplexObject complexObject = ComplexObject.newBuilder()
                    ...
                    .build();
                SerializedObject<byte[]> serialized = avroSerializer.serialize(complexObject, byte[].class);
                SerializationException serializationException = assertThrows(SerializationException.class, () -> avroSerializer.deserialize(serialized));
