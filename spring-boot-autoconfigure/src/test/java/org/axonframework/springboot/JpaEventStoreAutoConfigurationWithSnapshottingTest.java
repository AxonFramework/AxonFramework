...
class JpaEventStoreAutoConfigurationWithSnapshottingTest {
    ...
    void snapshotterAndSnapshotTriggerDefinitionAreInvoked() {
        testContext.run(context -> {
            ...
            commandGateway.send(new TestContext.CreateCommand(AGGREGATE_ID), ProcessingContext.empty())
                          .getResultMessage().get();
            commandGateway.send(new TestContext.UpdateCommand(AGGREGATE_ID), ProcessingContext.empty())
                          .getResultMessage().get();
