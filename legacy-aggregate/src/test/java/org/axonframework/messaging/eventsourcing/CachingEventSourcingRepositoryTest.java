...
import org.axonframework.messaging.eventsourcing.utils.StubAggregate;
...
class CachingEventSourcingRepositoryTest {
    ...
    @BeforeEach
    void setUp() {
        mockEventStore = spy(new StorageEngineBackedEventStore(new InMemoryEventStorageEngine(), new SimpleEventBus(), new AnnotationBasedTagResolver()));
...
//        }
    ...
    @Test
    void cacheClearedAfterRollbackOfLoadedAggregateUsingLoadOrCreate() {
        startAndGetUnitOfWork().executeWithResult((ctx) -> testSubject.newInstance(() -> new StubAggregate("id1")));
    ...
    @Test
    void cacheClearedAfterRollbackOfCreatedAggregateUsingLoadOrCreate() {
        LegacyUnitOfWork<?> uow = startAndGetUnitOfWork();
        uow.onCommit(c -> {
            throw new MockException();
        });
