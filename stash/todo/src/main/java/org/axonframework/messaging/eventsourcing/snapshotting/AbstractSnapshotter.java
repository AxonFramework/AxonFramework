...
/**
 * Abstract implementation of the {@link Snapshotter} that uses a task executor to
...
public abstract class AbstractSnapshotter implements Snapshotter {
    ...
    private void doScheduleSnapshot(Class<?> aggregateType, String aggregateIdentifier) {
        ...
        if (snapshotsInProgress.add(typeAndId)) {
            Span span = spanFactory.createScheduleSnapshotSpan(aggregateType.getSimpleName(), aggregateIdentifier)
                                   .start();
            try (SpanScope unused = span.makeCurrent()) {
                Span internalSpan = spanFactory.createCreateSnapshotSpan(aggregateType.getSimpleName(),
                                                                         aggregateIdentifier);
     ...
     * {@link DefaultSnapshotterSpanFactory} with a {@link NoOpSpanFactory} delegate. The {@link EventStore} is a
    ...
    public abstract static class Builder {
        ...
        private SnapshotterSpanFactory builderSpanFactory = DefaultSnapshotterSpanFactory.builder().spanFactory(
                NoOpSpanFactory.INSTANCE).build();
        ...
        /**
         * Sets the {@link SnapshotterSpanFactory} implementation to use for providing tracing capabilities. Defaults to
         * a {@link DefaultSnapshotterSpanFactory} with a {@link NoOpSpanFactory} by default, which provides no tracing
         * capabilities.
         *
        ...
        public void run() {
            try {
                ...
                if (logger.isDebugEnabled()) {
                    ...
                    logger.warn(
                            "An attempt to create and store a snapshot resulted in an exception. Exception summary: {}",
                            e.getMessage());
                }
            }
        }
