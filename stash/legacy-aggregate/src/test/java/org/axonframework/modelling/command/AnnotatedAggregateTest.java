...
public class AnnotatedAggregateTest {
    ...
    private static final TypeReference<Aggregate<AggregateRoot>> AGGREGATE_OF_ROOT = new TypeReference<>() {
    };
    ...
    void applyingMultipleEventsInAndThenPublishesWithRightState() {
        ...
        Aggregate<AggregateRoot> aggregate =
                uow.executeWithResult((ctx) -> repository.newInstance(() -> {
                   ...
                   .payloadAs(AGGREGATE_OF_ROOT);
        assertNotNull(aggregate);

        InOrder inOrder = inOrder(eventBus);
        inOrder.verify(eventBus).publish(eq(null), argThat((ArgumentMatcher<EventMessage>) x -> Event_1.class
    ...
    void applyingEventInHandlerPublishesInRightOrder() {
        ...
        inOrder.verify(eventBus).publish(eq(null), argThat((ArgumentMatcher<EventMessage>) x -> Event_2.class
                .equals(x.payloadType())));
    }
    ...
    void conditionalApplyingEventInHandlerPublishesInRightOrder(boolean applyConditional) {
        ...
        inOrderEvents.verify(eventBus).publish(eq(null), argThat((ArgumentMatcher<EventMessage>) x -> Event_1.class
                .equals(x.payloadType())));
        if (applyConditional) {
            inOrderEvents.verify(eventBus).publish(eq(null), argThat((ArgumentMatcher<EventMessage>) x -> Event_2.class
                    .equals(x.payloadType())));
        }
        inOrderEvents.verify(eventBus).publish(eq(null), argThat((ArgumentMatcher<EventMessage>) x -> Event_3.class
                .equals(x.payloadType())));
