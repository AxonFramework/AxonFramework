...
public class AxonServerSnapshotStore implements SnapshotStore {
    private static final String POSITION_TYPE_KEY = "__AxonFramework-Position-Type__";  // reserved key in metadata
     ...
     */
    public AxonServerSnapshotStore(
        @Nonnull AxonServerConnection connection,
    ...
    @Override
    @Nonnull
    public CompletableFuture<Void> store(@Nonnull QualifiedName qualifiedName, @Nonnull Object identifier, @Nonnull Snapshot snapshot) {
    ...
    @Override
    @Nonnull
    public CompletableFuture<Snapshot> load(@Nonnull QualifiedName qualifiedName, @Nonnull Object identifier) {
        Objects.requireNonNull(qualifiedName, "The qualifiedName parameter must not be null.");
        Objects.requireNonNull(identifier, "The identifier parameter must not be null.");
        ...
        return connection.snapshotChannel()
            ...
            .exceptionally(e -> {
                if (e instanceof StatusRuntimeException sre && (sre.getStatus() == Status.CANCELLED || sre.getStatus() == Status.NOT_FOUND)) {
